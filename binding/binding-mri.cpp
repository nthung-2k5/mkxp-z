...
ScriptBinding scriptBindingImpl = {mriBindingExecute, mriBindingTerminate, mriBindingReset};
...
#ifdef __WIN32__
    if (shState->config().winConsole) configureWindowsStreams();
#endif
}

static void showMsg(const std::string& msg) { shState->eThread().showMessageBox(msg.c_str()); }
...
RB_METHOD(mkxpDesensitize)
{
    ...
    return rb_utf8_str_new_cstr(shState->fileSystem().desensitize(RSTRING_PTR(filename)));
}
...
RB_METHOD(mkxpPlatform)
{
    ...
    if (mkxp_sys::isRosetta()) platform += " (Rosetta)";
...
RB_METHOD(mkxpFileExists)
{
    ...
    if (shState->fileSystem().exists(RSTRING_PTR(path))) return Qtrue;
    return Qfalse;
}
...
json5pp::value loadUserSettings()
{
    ...
    if (!ret.is_object()) ret = json5pp::object({});
...
RB_METHOD(mkxpGetJSONSetting)
{
    ...
    if (s[RSTRING_PTR(sname)].is_null()) { return json2rb(shState->config().raw.as_object()[RSTRING_PTR(sname)]); }
...
RB_METHOD(mriRgssMain)
{
    ...
    while (true)
    {
        VALUE exc = Qnil;
#if RAPI_FULL < 270
        rb_rescue2((VALUE(*)(ANYARGS))rgssMainCb, rb_block_proc(), (VALUE(*)(ANYARGS))rgssMainRescue, (VALUE)&exc,
                   rb_eException, (VALUE)0);
#else
        rb_rescue2(rgssMainCb, rb_block_proc(), rgssMainRescue, (VALUE)&exc, rb_eException, (VALUE)0);
#endif
...
static void runCustomScript(const std::string& filename)
{
    ...
    evalString(newStringUTF8(scriptData.c_str(), scriptData.size()), newStringUTF8(filename.c_str(), filename.size()),
               NULL);
}
...
static void runRMXPScripts(BacktraceData& btData)
{
    ...
    for (std::vector<std::string>::const_iterator i = conf.preloadScripts.begin(); i != conf.preloadScripts.end(); ++i)
        runCustomScript(*i);
...
static void mriBindingExecute()
{
    ...
    else { node = ruby_options(rubyArgsC.size(), const_cast<char**>(rubyArgsC.data())); }

    int state;
    bool valid = ruby_executable_node(node, &state);
    if (valid) state = ruby_exec_node(node);
    if (state || !valid)
    {
    ...
    else { rb_ary_push(lpaths, rb_utf8_str_new_cstr(mkxp_fs::getCurrentDirectory().c_str())); }
#endif
    ...
    VALUE exc = rb_gv_get("$!");
#endif
    if (!NIL_P(exc) && !rb_obj_is_kind_of(exc, rb_eSystemExit)) showExc(exc, btData);
