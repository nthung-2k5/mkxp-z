...
{
    if (RB_TYPE_P(obj, RUBY_T_STRING)) { out.push_back(RSTRING_PTR(obj)); }
...
RB_METHOD(fontDoesExist)
{
    ...
    if (RB_TYPE_P(nameObj, RUBY_T_STRING)) name = rb_string_value_cstr(&nameObj);
...
RB_METHOD(fontInitialize)
{
    ...
    if (rgssVer >= 3) wrapProperty(self, &f->getOutColor(), "out_color", ColorType);
...
void fontBindingInit()
{
    ...
    if (defNames.size() == 1) { defNamesObj = rb_utf8_str_new_cstr(defNames[0].c_str()); }
    else
    {
    ...
    if (rgssVer >= 3) wrapProperty(klass, &Font::getDefaultOutColor(), "default_out_color", ColorType);
    ...
    if (rgssVer >= 2) { INIT_KLASS_PROP_BIND(Font, DefaultShadow, "default_shadow"); }
    ...
    if (rgssVer >= 2) { INIT_PROP_BIND(Font, Shadow, "shadow"); }
