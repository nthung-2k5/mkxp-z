...
static int getButtonArg(VALUE* argv)
{
    ...
    if (FIXNUM_P(*argv)) { num = FIX2INT(*argv); }
...
RB_METHOD(inputControllerName)
{
    ...
    if (!shState->input().getControllerConnected()) return rb_utf8_str_new_cstr("");
...
RB_METHOD(inputControllerPowerLevel)
{
    ...
    if (!shState->input().getControllerConnected()) ret = M_SYMBOL("UNKNOWN");
