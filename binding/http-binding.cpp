...
bool strContainsStr(std::string& first, std::string second) { return first.find(second) != std::string::npos; }

VALUE getResponseBody(mkxp_net::HTTPResponse& res)
{
#if RAPI_FULL >= 190
    auto it = res.headers().find("Content-Type");
    if (it == res.headers().end()) return rb_str_new(res.body().c_str(), res.body().length());
...
void* httpGetInternal(void* req)
{
    ...
    GUARD_EXC(mkxp_net::HTTPResponse res = ((mkxp_net::HTTPRequest*)req)->get(); ret = formResponse(res););
...
void* httpPostInternal(void* args)
{
    ...
    GUARD_EXC(mkxp_net::HTTPResponse res = req->post(*postData); ret = formResponse(res););
...
void* httpPostBodyInternal(void* args)
{
    ...
    GUARD_EXC(mkxp_net::HTTPResponse res = req->post(reqbody, reqctype); ret = formResponse(res););
...
{
    if (v.is_null()) return Qnil;

    if (v.is_number()) return rb_float_new(v.as_number());

    if (v.is_string()) return rb_utf8_str_new_cstr(v.as_string().c_str());

    if (v.is_boolean()) return rb_bool_new(v.as_boolean());

    if (v.is_integer()) return LL2NUM(v.as_integer());
...
{
    if (v == Qnil) return json5pp::value(nullptr);

    if (RB_TYPE_P(v, RUBY_T_FLOAT)) return json5pp::value(RFLOAT_VALUE(v));

    if (RB_TYPE_P(v, RUBY_T_STRING)) return json5pp::value(RSTRING_PTR(v));

    if (v == Qtrue || v == Qfalse) return json5pp::value(RTEST(v));

    if (RB_TYPE_P(v, RUBY_T_FIXNUM)) return json5pp::value(NUM2DBL(v));
