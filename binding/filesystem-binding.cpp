...
void call_RWread_cb(fileIntReadCbArgs* args) { SDL_RWread(args->ops, args->dst, 1, args->length); }
#endif
...
kernelLoadDataInt(const char* filename, bool rubyExc, bool raw)
{
    ...
    else { result = fileIntRead(0, 0, port); }
...
#endif
{
    if (RB_TYPE_P(arg, RUBY_T_STRING) && ENCODING_IS_ASCII8BIT(arg)) rb_enc_associate_index(arg, rb_utf8_encindex());
