...
#endif
    if (!hfunc) rb_raise(rb_eRuntimeError, "%s", SDL_GetError());
...
#define params param.params
    ...
    if (items != nimport) rb_raise(rb_eRuntimeError, "wrong number of parameters: expected %d, got %d", nimport, items);
    ...
    for (int i = 0; i < nimport; i++)
    {
        ...
        {
        case _T_POINTER:
            if (NIL_P(str)) { lParam = 0; }
            else if (FIXNUM_P(str)) { lParam = RB2MVAL(str); }
            else
            {
                StringValue(str);
                rb_str_modify(str);
