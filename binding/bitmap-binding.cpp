...
RB_METHOD(bitmapStretchBlt)
{
    ...
    GFX_GUARD_EXC(b->stretchBlt(destRect->toIntRect(), *src, srcRect->toIntRect(), opacity););
...
RB_METHOD(bitmapDrawText)
{
    ...
    if (argc == 2 || argc == 3)
    {
        ...
        else { rb_get_args(argc, argv, "oz|i", &rectObj, &str, &align RB_ARG_END); }
    ...
    else
    {
        ...
        else { rb_get_args(argc, argv, "iiiiz|i", &x, &y, &width, &height, &str, &align RB_ARG_END); }
...
RB_METHOD(bitmapTextSize)
{
    ...
    else { rb_get_args(argc, argv, "z", &str RB_ARG_END); }
...
RB_METHOD(bitmapGradientFillRect)
{
    ...
    if (argc == 3 || argc == 4)
    {
        ...
        GFX_GUARD_EXC(b->gradientFillRect(rect->toIntRect(), color1->norm, color2->norm, vertical););
    }
    else
    {
        int x, y, width, height;
        ...
        GFX_GUARD_EXC(b->gradientFillRect(x, y, width, height, color1->norm, color2->norm, vertical););
    }
...
RB_METHOD(bitmapSetFPS)
{
    ...
    GFX_GUARD_EXC(
        if (RB_TYPE_P(fps, RUBY_T_FLOAT)) { b->setAnimationFPS(RFLOAT_VALUE(fps)); } else {
