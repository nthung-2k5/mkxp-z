...
struct
{
    RbException id;
    const char* name;
} static customExc[] = {{MKXP, "MKXPError"}, {PHYSFS, "PHYSFSError"}, {SDL, "SDLError"}};
...
int rb_get_args(int argc, VALUE* argv, const char* format, ...)
{
    ...
    while ((c = *format++))
    {
        ...
        default:
            ...
            if (argc <= argI && !opt) rb_raise(rb_eArgError, "wrong number of arguments");
        ...
        case 'S': {
            ...
            if (!RB_TYPE_P(tmp, RUBY_T_STRING)) rb_raise(rb_eTypeError, "Argument %d: Expected string", argI);
        ...
        case 'n': {
            ...
            if (!SYMBOL_P(symVal)) rb_raise(rb_eTypeError, "Argument %d: Expected symbol", argI);
    ...
    // FIXME print num of needed args vs provided
    if (!c && argc > argI) rb_raise(rb_eArgError, "wrong number of arguments");
