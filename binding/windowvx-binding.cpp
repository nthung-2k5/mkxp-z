...
RB_METHOD(windowVXInitialize)
{
    ...
    if (rgssVer >= 3)
    {
        ...
        if (argc == 4) rb_get_args(argc, argv, "iiii", &x, &y, &width, &height RB_ARG_END);

        w = new WindowVX(x, y, width, height);
    }
    else { w = viewportElementInitialize<WindowVX>(argc, argv, self); }
    ...
    if (rgssVer >= 3) wrapProperty(self, &w->getTone(), "tone", ToneType);
