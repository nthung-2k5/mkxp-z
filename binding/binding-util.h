...
#define DEF_TYPE_CUSTOMFREE(Klass, Free) DEF_TYPE_CUSTOMNAME_AND_FREE(Klass, #Klass, Free)

#define DEF_TYPE_CUSTOMNAME(Klass, Name) DEF_TYPE_CUSTOMNAME_AND_FREE(Klass, Name, freeInstance<Klass>)
...
#define DEF_ALLOCFUNC_CUSTOMFREE(type, free)                                                                           \
    static VALUE type##Allocate(VALUE klass) { return Data_Wrap_Struct(klass, 0, free, 0); }
...
template <class C>
inline C* getPrivateData(VALUE self)
{
#if RAPI_FULL > 187
...
#endif
    if (!c) { raiseRbExc(Exception(Exception::MKXPError, "No instance data for variable (missing call to super?)")); }
    return c;
}
...
{
    if (actual != expected) rb_raise(rb_eArgError, "wrong number of arguments (%d for %d)", actual, expected);
}

#if RAPI_MAJOR < 2
static inline void rb_error_arity(int argc, int min, int max)
{
    if (argc > max || argc < min) rb_raise(rb_eArgError, "Finish me! rb_error_arity()"); // TODO
}
...
#define DEF_PROP_I(Klass, PropName) DEF_PROP(Klass, int, PropName, int, rb_fix_new)

#define DEF_PROP_F(Klass, PropName) DEF_PROP(Klass, double, PropName, float, rb_float_new)

#define DEF_PROP_B(Klass, PropName) DEF_PROP(Klass, bool, PropName, bool, rb_bool_new)
...
#else
#define DEF_GFX_PROP_OBJ_REF(Klass, PropKlass, PropName, prop_iv) DEF_PROP_OBJ_REF(Klass, PropKlass, PropName, prop_iv)
#endif
...
#else
#define DEF_GFX_PROP_OBJ_VAL(Klass, PropKlass, PropName, prop_iv) DEF_PROP_OBJ_VAL(Klass, PropKlass, PropName, prop_iv)
#endif
...
#define DEF_GFX_PROP_I(Klass, PropName) DEF_GFX_PROP(Klass, int, PropName, int, rb_fix_new)

#define DEF_GFX_PROP_F(Klass, PropName) DEF_GFX_PROP(Klass, double, PropName, float, rb_float_new)

#define DEF_GFX_PROP_B(Klass, PropName) DEF_GFX_PROP(Klass, bool, PropName, bool, rb_bool_new)
