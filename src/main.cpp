...
static inline const char* glGetStringInt(GLenum name) { return (const char*)gl.GetString(name); }
...
int main(int argc, char* argv[])
{
    ...
    char* tmp{};
    tmp = getenv("SRCDIR");
    if (tmp) { strncpy(dataDir, tmp, sizeof(dataDir)); }
#endif
    ...
    if (conf.winConsole)
    {
        if (setupWindowsConsole()) { reopenWindowsStreams(); }
        else
        {
            char buf[200];
    ...
    if (conf.windowTitle.empty()) conf.windowTitle = conf.game.title;
    ...
    if (conf.winResizable) winFlags |= SDL_WINDOW_RESIZABLE;
    ...
    win = SDL_CreateWindow(conf.windowTitle.c_str(), SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, conf.defScreenW,
...
#ifdef MKXPZ_BUILD_XCODE
    {
        ...
        {
            showInitError(conf.game.title + " cannot run from the Downloads directory.\n\n" +
...
#define DEBUG_FSELECT_PROMPT "Load Game"
    ...
    if (!mode.refresh_rate) conf.syncToRefreshrate = false;
    ...
    RGSSThreadData rtData(&eventThread, argv[0], win, alcDev, mode.refresh_rate, mkxp_sys::getScalingFactor(), conf,
                          glCtx);
    ...
    if (!rtData.rgssErrorMsg.empty())
    {
        ...
        SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, conf.game.title.c_str(), rtData.rgssErrorMsg.c_str(), win);
    }
...
static SDL_GLContext initGL(SDL_Window* win, Config& conf, RGSSThreadData* threadData)
{
    ...
    if (conf.debugMode) SDL_GL_SetAttribute(SDL_GL_CONTEXT_FLAGS, SDL_GL_CONTEXT_DEBUG_FLAG);
