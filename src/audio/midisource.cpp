...
static void badMidiFormat() { throw Exception(Exception::MKXPError, "Midi: Bad format"); }
...
struct MemChunk
{
    ...
    MemChunk(const std::vector<uint8_t>& data): data(data), i(0) {}

    uint8_t readByte()
    {
        if (i >= data.size()) endOfFile();
    ...
    void endOfFile() { throw Exception(Exception::MKXPError, "Midi: EOF"); }
};

static uint32_t readVarNum(MemChunk& chunk)
{
    uint32_t result = 0;
    uint8_t byte;
    uint8_t len = 0;
...
static void readVoiceEvent(MidiEvent& e, MemChunk& chunk, uint8_t type, uint8_t data1, bool& handled)
{
...
static void readEvent(MidiReadHandler* handler, MemChunk& chunk, uint8_t& prevType, uint32_t& deltaBase,
                      uint32_t& deltaCarry, bool& endOfTrack)
{
    MidiEvent e;
    bool handled = true;
    ...
    else if (type == 0xFF)
    {
        ...
        {
            /* Tempo event */
            if (len != 3) badMidiFormat();

            uint8_t data[3];
            chunk.readData(data, 3);

            uint32_t mpqn = (data[0] << 0x10) | (data[1] << 0x08) | (data[2] << 0x00);
...
void readMidiTrack(MidiReadHandler* handler, MemChunk& chunk)
{
    ...
    if (strcmp(sig, "MTrk")) badMidiFormat();
    ...
    if ((chunk.i - savedPos) != trackLen) badMidiFormat();
}
...
struct Track
{
    ...
    Track():
        length(0), loopI(-1), loopOffsetEnd(0), loopOffsetStart(0), valid(false), index(0), wrapAroundFlag(false),
        atEnd(false)
    {
    }
...
struct CCResetter
{
    ...
    CCResetter() { memset(&chanHandled, 0, sizeof(chanHandled)); }
...
struct MidiSource: ALDataSource, MidiReadHandler
{
    ...
    MidiSource(SDL_RWops& ops, bool looped):
        freq(SYNTH_SAMPLERATE), looped(looped), dpb(480), pitchShift(0), genDeltasCarry(0), curTrack(-1)
    {
        ...
        if (loopDelta >= longest) loopDelta = 0;
    ...
    ~MidiSource() { shState->midiState().releaseSynth(synth); }
    ...
    {
        if (midiType != 0 && midiType != 1) throw Exception(Exception::MKXPError, "Midi: Type 2 not supported");
    ...
    void onMidiTrackBegin() { ++curTrack; }
    ...
    void onMidiEvent(const MidiEvent& e, uint32_t absDelta)
    {
        ...
        if (e.type == CC && e.e.cc.ctrl == CC_CTRL_LOOP) loopDelta = absDelta;
    }
    ...
    Status fillBuffer(AL::Buffer::ID buf)
    {
        ...
        while (remTicks > 0)
        {
            ...
            for (size_t i = 0; i < tracks.size(); ++i)
            {
                ...
                while (true)
                {
                    if (!track.valid || track.remDeltas > 0) break;
                    ...
                    if (prevOffset < 0) track.remDeltas += prevOffset;

                    /* Ensure it lies in the future */
                    if (track.remDeltas > 0) break;
                }
            }
            ...
            for (size_t i = 0; i < tracks.size(); ++i)
            {
                 ...
                 * to become current */
                if (remDelta < nextEvent) nextEvent = std::max<uint32_t>(remDelta, 1);
            }
            ...
            for (size_t i = 0; i < tracks.size(); ++i)
                if (tracks[i].valid) tracks[i].remDeltas -= intDeltas;
        }

        /* Fill AL buffer */
        AL::Buffer::uploadData(buf, AL_FORMAT_STEREO16, synthBuf, sizeof(synthBuf), freq);

        if (tracks[longestI].atEnd) return EndOfStream;

        return NoError;
    }

    int sampleRate() { return freq; }
...
ALDataSource* createMidiSource(SDL_RWops& ops, bool looped) { return new MidiSource(ops, looped); }
