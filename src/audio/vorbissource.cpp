...
static long vfTell(void* ops) { return SDL_RWtell(static_cast<SDL_RWops*>(ops)); }

static ov_callbacks OvCallbacks = {vfRead, vfSeek, 0, vfTell};
...
struct VorbisSource: ALDataSource
{
    ...
    VorbisSource(SDL_RWops& ops, bool looped): src(ops), currentFrame(0)
    {
        ...
        for (int i = 0; i < vf.vc->comments; ++i)
        {
            ...
            *sep = '\0';
            ...
            if (!strcmp(comment, "LOOPSTART")) loop.start = strtol(sep + 1, 0, 10);

            if (!strcmp(comment, "LOOPLENGTH")) loop.length = strtol(sep + 1, 0, 10);
    ...
    int sampleRate() { return info.rate; }
    ...
    void seekToOffset(float seconds)
    {
        ...
        if (loop.valid && currentFrame > loop.end) currentFrame = loop.start;

        /* If seeking fails, just seek back to start */
        if (ov_pcm_seek(&vf, currentFrame) != 0) ov_raw_seek(&vf, 0);
    }
    ...
    Status fillBuffer(AL::Buffer::ID alBuffer)
    {
        ...
        while (canRead > 16)
        {
            ...
            if (res == 0)
            {
                ...
                else { retStatus = ALDataSource::EndOfStream; }
            ...
            if (loop.valid && currentFrame >= loop.end)
            {
                ...
                if (ov_pcm_seek(&vf, currentFrame) != 0) retStatus = ALDataSource::Error;
...
ALDataSource* createVorbisSource(SDL_RWops& ops, bool looped) { return new VorbisSource(ops, looped); }
