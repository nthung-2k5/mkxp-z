...
struct SoundBuffer
{
    ...
    SoundBuffer(): link(this), refCount(1) { alBuffer = AL::Buffer::gen(); }
  ...
  private:
    ~SoundBuffer() { AL::Buffer::del(alBuffer); }
};
...
SoundEmitter::SoundEmitter(const Config& conf):
    bufferBytes(0), srcCount(conf.SE.sourceCount), alSrcs(srcCount), atchBufs(srcCount), srcPrio(srcCount)
{
...
void SoundEmitter::play(const std::string& filename, int volume, int pitch)
{
    ...
    if (switchBuffer) AL::Source::detachBuffer(src);

    SoundBuffer* old = atchBufs[srcIndex];

    if (old) SoundBuffer::deref(old);

    atchBufs[srcIndex] = SoundBuffer::ref(buffer);

    if (switchBuffer) AL::Source::attachBuffer(src, buffer->alBuffer);
...
struct SoundOpenHandler: FileSystem::OpenHandler
{
    ...
    bool tryRead(SDL_RWops& ops, const char* ext)
    {
        ...
        AL::Buffer::uploadData(buffer->alBuffer, alFormat, sample->buffer, buffer->bytes, sample->actual.rate);
