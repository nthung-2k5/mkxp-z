...
void AudioStream::play(const std::string& filename, int volume, int pitch, float offset)
{
    ...
    if (filename == current.filename && _volume == current.volume && _pitch == current.pitch &&
    ...
    if (filename == current.filename && _pitch == current.pitch &&
        (sState == ALStream::Playing || sState == ALStream::Paused))
    {
...
void AudioStream::lockStream() { SDL_LockMutex(streamMut); }

void AudioStream::unlockStream() { SDL_UnlockMutex(streamMut); }
...
float AudioStream::getVolume(VolumeType type) { return volumes[type]; }

float AudioStream::playingOffset() { return stream.queryOffset(); }
...
void AudioStream::startFadeIn()
{
    ...
    fadeIn.thread = createSDLThread<AudioStream, &AudioStream::fadeInThread>(this, fadeIn.threadName);
}
