...
void ALStream::setVolume(float value) { AL::Source::setVolume(alSrc, value); }
...
{
    if (state == Closed || !source) return 0;
...
void ALStream::closeSource() { delete source; }
...
struct ALStreamOpenHandler: FileSystem::OpenHandler
{
    ...
    ALStreamOpenHandler(SDL_RWops& srcOps, bool looped): srcOps(&srcOps), looped(looped), source(0) {}
    ...
    while (true)
    {
        ...
        while (procBufs--)
        {
            ...
            else
            {
                ...
                if (bits != 0 && chan != 0) procFrames += ((size / (bits / 8)) / chan);
            }
             ...
             * start playing again */
            if (AL::Source::getState(alSrc) == AL_STOPPED) AL::Source::play(alSrc);
            ...
            if (status == ALDataSource::WrapAround) lastBuf = buf;

            if (status == ALDataSource::EndOfStream) sourceExhausted.set();
        }
