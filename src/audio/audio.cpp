...
struct AudioPrivate
{
    ...
    AudioPrivate(RGSSThreadData& rtData):
        bgs(ALStream::Looped, "bgs"), me(ALStream::NotLooped, "me"), se(rtData.config), syncPoint(rtData.syncPoint),
        volumeRatio(1)
    {
    ...
    AudioStream* getTrackByIndex(int index)
    {
        ...
        {
            throw Exception(Exception::MKXPError, "requested BGM track %d out of range (max: %d)", index,
                            bgmTracks.size() - 1);
        }
                ...
                for (int i = 0; i < (int)(bgmTracks.size()); i++)
                {
                    ...
                    if (vol < 0 || track->stream.queryState() != ALStream::Playing)
                    {
                        ...
                        for (auto t: bgmTracks)
                            if (t->stream.queryState() == ALStream::Playing) playingTracks.push_back(t);
                ...
                if (me.stream.queryState() != ALStream::Playing)
                {
                    ...
                    for (auto track: bgmTracks)
                    {
                        ...
                        else
                        {
                            ...
                            if (!track->noResumeStop) track->stream.play();
...
Audio::Audio(RGSSThreadData& rtData): p(new AudioPrivate(rtData)) {}

void Audio::bgmPlay(const char* filename, int volume, int pitch, float pos, int track)
{
    if (track == -127)
    {
...
{
    if (track == -127) return p->bgmTracks[0]->getVolume(AudioStream::BaseRatio) * 100;
...
void Audio::bgsStop() { p->bgs.stop(); }

void Audio::bgsFade(int time) { p->bgs.fadeOut(time); }

void Audio::mePlay(const char* filename, int volume, int pitch) { p->me.play(filename, volume, pitch); }

void Audio::meStop() { p->me.stop(); }

void Audio::meFade(int time) { p->me.fadeOut(time); }

void Audio::sePlay(const char* filename, int volume, int pitch) { p->se.play(filename, volume, pitch); }

void Audio::seStop() { p->se.stop(); }

void Audio::setupMidi() { shState->midiState().initIfNeeded(shState->config()); }

float Audio::bgmPos(int track) { return p->getTrackByIndex(track)->playingOffset(); }

float Audio::bgsPos() { return p->bgs.playingOffset(); }
