...
struct SharedStatePrivate
{
    ...
    SharedStatePrivate(RGSSThreadData* threadData):
        bindingData(0), sdlWindow(threadData->window), fileSystem(threadData->argv0, threadData->config.allowSymlinks),
        eThread(*threadData->ethread), rtData(*threadData), config(threadData->config), midiState(threadData->config),
        graphics(threadData), input(*threadData), audio(*threadData), _glState(threadData->config),
        fontState(threadData->config), stampCounter(0)
    {

        startupTime = std::chrono::steady_clock::now();

        /* Shaders have been compiled in ShaderSet's constructor */
        if (gl.ReleaseShaderCompiler) gl.ReleaseShaderCompiler();
        ...
        if (config.pathCache) fileSystem.createPathCache();
        ...
        if (rgssVer <= 2) midiState.initIfNeeded(threadData->config);
    }
...
void SharedState::setScreen(Scene& screen) { p->screen = &screen; }

#define GSATT(type, lower)                                                                                             \
    type SharedState ::lower() const { return p->lower; }
...
void SharedState::setBindingData(void* data) { p->bindingData = data; }

void SharedState::ensureQuadIBO(size_t minSize) { _globalIBO->ensureSize(minSize); }

GlobalIBO& SharedState::globalIBO() { return *_globalIBO; }
...
Font& SharedState::defaultFont() const { return *p->defaultFont; }
...
unsigned int SharedState::genTimeStamp() { return p->stampCounter++; }
...
SharedState::~SharedState() { delete p; }
