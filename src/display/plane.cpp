...
struct PlanePrivate
{
    ...
    PlanePrivate():
        bitmap(0), opacity(255), blendType(BlendNormal), color(&tmp.color), tone(&tmp.tone), ox(0), oy(0), zoomX(1),
        zoomY(1), quadSourceDirty(false)
    {
        prepareCon = shState->prepareDraw.connect(&PlanePrivate::prepare, this);

        qArray.resize(1);
    }

    ~PlanePrivate() { prepareCon.disconnect(); }
...
void Plane::draw()
{
    ...
    if (gl.npot_repeat) TEX::setRepeat(true);

    p->qArray.draw();

    if (gl.npot_repeat) TEX::setRepeat(false);
...
{
    if (gl.npot_repeat) Quad::setPosRect(&p->qArray.vertices[0], FloatRect(geo.rect));
