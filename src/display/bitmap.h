...
class Bitmap: public Disposable
{
  public:
    ...
    void blt(int x, int y, const Bitmap& source, const IntRect& rect, int opacity = 255);

    void stretchBlt(IntRect destRect, const Bitmap& source, IntRect sourceRect, int opacity = 255);

    void fillRect(int x, int y, int width, int height, const Vec4& color);
    void fillRect(const IntRect& rect, const Vec4& color);

    void gradientFillRect(int x, int y, int width, int height, const Vec4& color1, const Vec4& color2,
                          bool vertical = false);
