...
#define EXC(msg) Exception(Exception::MKXPError, "%s", msg)
...
#define EXT_SUFFIX ""
    ...
    else if (gles && glMajor == 2) { GL_FBO_FUN; }
...
#define EXT_SUFFIX "EXT"
        ...
        if (HAVE_EXT(EXT_framebuffer_blit)) { GL_FBO_BLIT_FUN; }
    }
    else { throw EXC("No FBO support available"); }
...
#define EXT_SUFFIX "GREMEDY"
    ...
    /* Misc caps */
    if (!gles || glMajor >= 3 || HAVE_EXT(EXT_unpack_subimage)) gl.unpack_subimage = true;

    if (!gles || glMajor >= 3 || HAVE_EXT(OES_texture_npot)) gl.npot_repeat = true;
}
