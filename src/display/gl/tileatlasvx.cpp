...
static const StaticRect autotileVXRectsC[] = {
    {32.5f, 0.5f, 15.0f, 31.0f}, {16.5f, 0.5f, 15.0f, 31.0f}, {0.0f, 0.5f, 15.0f, 31.0f}, {16.5f, 0.5f, 15.0f, 31.0f},
    {32.5f, 0.5f, 15.0f, 31.0f}, {48.5f, 0.5f, 15.0f, 31.0f}, {0.0f, 0.5f, 15.0f, 31.0f}, {48.5f, 0.5f, 15.0f, 31.0f}};
...
namespace TileAtlasVX
{
    ...
    static int16_t tableGetSafe(const Table* t, int x)
    {
        if (!t) return 0;
    ...
    struct Blit
    {
        ...
        Blit(int srcX, int srcY, int w, int h, int dstX, int dstY): src(srcX, srcY, w, h), dst(dstX, dstY) {}

        Blit(int srcX, int srcY, const Size& size, int dstX, int dstY): src(srcX, srcY, size.w, size.h), dst(dstX, dstY)
        {
        }
    };
    ...
    static const Blit blitsA1[] = {
        ...
        Blit(0, 0, 6, 12, 0, 0), Blit(8, 0, 6, 12, 6, 0),
        /* Unanimated A autotiles */
        Blit(6, 0, 2, 6, freeArea.x, freeArea.y),
        /* C autotiles */
        Blit(14, 0, 2, 6, 12, 0), Blit(6, 6, 2, 6, 14, 0), Blit(14, 6, 2, 6, 12, 6)};

    static const Blit blitsA2[] = {Blit(0, 0, bmSizes[BM_A2], 0, atArea.h)};

    static const Blit blitsA3[] = {Blit(0, 0, bmSizes[BM_A3], 0, blitsA2[0].dst.y + blitsA2[0].src.h)};

    static const Blit blitsA4[] = {Blit(0, 0, bmSizes[BM_A4], 0, blitsA3[0].dst.y + blitsA3[0].src.h)};

    static const Blit blitsA5[] = {Blit(0, 0, bmSizes[BM_A5], 0, blitsA4[0].dst.y + blitsA4[0].src.h)};

    static const Blit blitsB[] = {Blit(0, 0, bmSizes[BM_B], atArea.w, 0)};

    static const Blit blitsC[] = {Blit(0, 0, bmSizes[BM_C], blitsA2[0].src.w, blitsB[0].dst.y + blitsB[0].src.h)};

    static const Blit blitsD[] = {Blit(0, 0, bmSizes[BM_D], blitsC[0].dst.x, blitsC[0].dst.y + blitsC[0].src.h)};

    static const Blit blitsE[] = {Blit(0, 0, bmSizes[BM_E], blitsD[0].dst.x, blitsD[0].dst.y + blitsD[0].src.h)};
    ...
    static const Vec2i AEPartsDst[] = {Vec2i(12, 0), Vec2i(12, 3), Vec2i(14, 0),
                                       Vec2i(14, 3), Vec2i(12, 6), Vec2i(12, 9)};

    static const Vec2i shadowArea(freeArea.x + 2, freeArea.y);

    static SDL_Surface* createShadowSet()
    {
        int bpp;
        Uint32 rm, gm, bm, am;
    ...
    static void doBlit(Bitmap* bm, const IntRect& src, const Vec2i& dst)
    {
        ...
        if (!SDL_IntersectRect(&_src, &bmr, &_src)) return;
    ...
    void build(TEXFBO& tf, Bitmap* bitmaps[BM_COUNT])
    {
        ...
        if (rgssVer >= 3)
        {
            ...
            if (tf.selfHires != nullptr)
            {
                ...
                SDL_Surface* blitTemp = SDL_CreateRGBSurface(0, destWidth, destHeight, bpp, rMask, gMask, bMask, aMask);

                SDL_BlitScaled(shadow, &srcRect, blitTemp, 0);

                TEX::bind(tf.selfHires->tex);
                TEX::uploadSubImage(destX, destY, blitTemp->w, blitTemp->h, blitTemp->pixels, GL_RGBA);
            }
            else
            {
                TEX::bind(tf.tex);
                TEX::uploadSubImage(shadowArea.x * 32, shadowArea.y * 32, shadow->w, shadow->h, shadow->pixels,
                                    GL_RGBA);
            }
...
#define TABLE_FLAG (1 << 7)
    ...
    static void readAutotile(Reader& reader, int patternID, const Vec2i& orig, int x, int y,
    ...
    static void readAutotileA(Reader& reader, int patternID, const Vec2i& orig, int x, int y)
    {
        readAutotile(reader, patternID, orig, x, y, autotileVXRectsA, autotileVXRectsAN);
    }

    static void readAutotileA2(Reader& reader, int patternID, const Vec2i& orig, int x, int y)
    {
    ...
    static void readAutotileB(Reader& reader, int patternID, const Vec2i& orig, int x, int y)
    {
        if (patternID >= 0x10) return;

        readAutotile(reader, patternID, orig, x, y, autotileVXRectsB, autotileVXRectsBN);
    }

    static void readAutotileC(Reader& reader, int patternID, const Vec2i& orig, int x, int y)
    {
        if (patternID > 0x3) return;
    ...
    static void onTileA1(Reader& reader, int16_t tileID, const int x, const int y)
    {
        tileID -= 0x0800;
        ...
        const Vec2i atOrig[] = {Vec2i(0, 0), Vec2i(0, 3), Vec2i(freeArea), Vec2i(freeArea.x, freeArea.y + 3),
                                Vec2i(6, 0), autotileC,   Vec2i(6, 3),     autotileC,

                                Vec2i(0, 6), autotileC,   Vec2i(0, 9),     autotileC,
                                Vec2i(6, 6), autotileC,   Vec2i(6, 9),     autotileC};
    ...
    static void onTileA2(Reader& reader, int16_t tileID, int x, int y, bool isTable)
    {
    ...
    static void onTileA3(Reader& reader, int16_t tileID, int x, int y)
    {
    ...
    static void onTileA4(Reader& reader, int16_t tileID, int x, int y)
    {
    ...
    static void onTileA5(Reader& reader, int16_t tileID, int x, int y, bool overPlayer)
    {
    ...
    static void onTileBCDE(Reader& reader, int16_t tileID, int x, int y, bool overPlayer)
    {
        int ox = tileID % 0x8;
    ...
    static void onTile(Reader& reader, int16_t tileID, int x, int y, const Table* flags)
    {
    ...
    static void readLayer(Reader& reader, const Table& data, const Table* flags, int ox, int oy, int w, int h, int z)
    {
    ...
    static void onShadowTile(Reader& reader, int8_t value, int x, int y)
    {
        if (value == 0) return;
    ...
    static void readShadowLayer(Reader& reader, const Table& data, int ox, int oy, int w, int h)
    {
        for (int y = 0; y < h; ++y)
            for (int x = 0; x < w; ++x)
            {
    ...
    void readTiles(Reader& reader, const Table& data, const Table* flags, int ox, int oy, int w, int h)
    {
        for (int i = 0; i < 2; ++i)
            readLayer(reader, data, flags, ox, oy, w, h, i);

        if (rgssVer >= 3) readShadowLayer(reader, data, ox, oy, w, h);
