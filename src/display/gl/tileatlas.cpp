...
namespace TileAtlas
{
    ...
    static int freeArea(int width, int height) { return width * height - atArea; }
    ...
    Vec2i minSize(int tilesetH, int maxAtlasSize)
    {
        ...
        if (freeArea(width, height) >= tsArea && height <= maxAtlasSize) return Vec2i(width, height);

        /* Expand horizontally */
        while (freeArea(width, height) < tsArea && width < maxAtlasSize)
            width += tsLaneW;

        if (freeArea(width, height) >= tsArea && width <= maxAtlasSize) return Vec2i(width, height);
        ...
        for (size_t srcI = 0; srcI < srcCols.size(); ++srcI)
        {
            ...
            for (; dstI < (int)dstCols.size() && srcCol.h > 0; ++dstI)
            {
                ...
                else if (srcCol.h < dstCol.h)
                {
                    ...
                    blits.push_back(Blit(srcCol.x, srcCol.y, dstCol.x, dstCol.y, srcCol.h));
