...
namespace GLMeta
{
                            ...
                            SDL_Surface* src, GLenum format)
    {
        ...
        else
        {
            ...
            SDL_Surface* tmp = SDL_CreateRGBSurface(0, dstW, dstH, form->BitsPerPixel, form->Rmask, form->Gmask,
                                                    form->Bmask, form->Amask);
...
#define HAVE_NATIVE_VAO gl.GenVertexArrays
    ...
    {
        if (HAVE_NATIVE_VAO) { gl.BindVertexArray(0); }
        else
        {
            for (size_t i = 0; i < vao.attrCount; ++i)
...
#define HAVE_NATIVE_BLIT gl.BlitFramebuffer
    ...
    void blitSource(TEXFBO& source)
    {
        ...
        if (HAVE_NATIVE_BLIT) { gl.BindFramebuffer(GL_READ_FRAMEBUFFER, source.fbo.gl); }
        else
        {
            switch (shState->config().smoothScaling)
            {
            ...
            if (source.selfHires != nullptr) { TEX::bind(source.selfHires->tex); }
            else { TEX::bind(source.tex); }
        }
    }
    ...
    void blitRectangle(const IntRect& src, const IntRect& dst, bool smooth)
    {
        ...
        else
        {
            ...
            {
                XbrzShader& shader = shState->shaders().xbrz;
                shader.setTargetScale(
                    Vec2((float)(shState->config().xbrzScalingFactor), (float)(shState->config().xbrzScalingFactor)));
            }
#endif
