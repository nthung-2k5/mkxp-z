...
static void applyBool(GLenum state, bool mode) { mode ? gl.Enable(state) : gl.Disable(state); }

void GLClearColor::apply(const Vec4& value) { gl.ClearColor(value.x, value.y, value.z, value.w); }
...
void GLScissorBox::apply(const IntRect& value)
{
    ...
    if (shState)
    {
        ...
        {
            gl.Scissor((int)lround(framebufferScalingFactor * value.x), (int)lround(framebufferScalingFactor * value.y),
                       (int)lround(framebufferScalingFactor * value.w),
                       (int)lround(framebufferScalingFactor * value.h));
        }
        else { gl.Scissor(value.x, value.y, value.w, value.h); }
    }
    else { gl.Scissor(value.x, value.y, value.w, value.h); }
}
...
void GLScissorTest::apply(const bool& value) { applyBool(GL_SCISSOR_TEST, value); }
...
void GLViewport::apply(const IntRect& value) { gl.Viewport(value.x, value.y, value.w, value.h); }
