...
_TTF_Font* SharedFontState::getFont(std::string family, int size)
{
    ...
    if (family.empty()) family = p->defaultFamily;
    ...
    if (!font) throw Exception(Exception::SDLError, "%s", SDL_GetError());
...
void SharedFontState::setDefaultFontFamily(const std::string& family) { p->defaultFamily = family; }
...
void pickExistingFontName(const std::vector<std::string>& names, std::string& out, const SharedFontState& sfs)
{
    ...
    for (size_t i = 0; i < names.size(); ++i)
    {
        ...
        {
            if (i == 0) { Debug() << "Primary font not found:" << names[i]; }
            else { Debug() << "Fallback font not found:" << names[i]; }
        }
    }
...
struct FontPrivate
{
    ...
    FontPrivate(int size):
        size(size), bold(defaultBold), italic(defaultItalic), outline(defaultOutline), shadow(defaultShadow),
        color(&colorTmp), outColor(&outColorTmp), colorTmp(*defaultColor), outColorTmp(*defaultOutColor), sdlFont(0),
        isSolid(false)
    {
    }

    FontPrivate(const FontPrivate& other):
        name(other.name), size(other.size), bold(other.bold), italic(other.italic), outline(other.outline),
        shadow(other.shadow), color(&colorTmp), outColor(&outColorTmp), colorTmp(*other.color),
        outColorTmp(*other.outColor), sdlFont(other.sdlFont), isSolid(false)
    {
    }
...
bool Font::isSolid() const { return p->isSolid; }
...
Font::Font(const Font& other) { p = new FontPrivate(*other.p); }
...
void Font::setSize(int value, bool checkIllegal)
{
    ...
    if (value < 6 || value > 96)
    {
        if (checkIllegal) { throw Exception(Exception::ArgumentError, "%s", "bad value for size"); }
    }
...
const std::vector<std::string>& Font::getInitialDefaultNames() { return FontPrivate::initialDefaultNames; }

void Font::initDynAttribs()
{
    p->color = new Color(p->colorTmp);

    if (rgssVer >= 3) p->outColor = new Color(p->outColorTmp);
    ;
}
...
void Font::initDefaultDynAttribs()
{
    ...
    if (rgssVer >= 3) FontPrivate::defaultOutColor = new Color(FontPrivate::defaultOutColorTmp);
}
...
{
    if (!p->sdlFont) p->sdlFont = shState->fontState().getFont(p->name.c_str(), p->size);
