...
static gif_result gif_initialise_sprite(gif_animation* gif, unsigned int width, unsigned int height)
{
    ...
    if ((width <= gif->width) && (height <= gif->height)) { return GIF_OK; }
    ...
    if (buffer == NULL) { return GIF_INSUFFICIENT_MEMORY; }
...
static gif_result gif_initialise_frame_extensions(gif_animation* gif, const int frame)
{
    ...
    {
        ++gif_data;
        if ((gif_bytes = (gif_end - gif_data)) < 1) { return GIF_INSUFFICIENT_FRAME_DATA; }
        ...
        case GIF_EXTENSION_APPLICATION:
            ...
            if ((gif_data[1] == 0x0b) && (strncmp((const char*)gif_data + 2, "NETSCAPE2.0", 11) == 0) &&
                (gif_data[13] == 0x03) && (gif_data[14] == 0x01))
            {
...
static gif_result gif_initialise_frame(gif_animation* gif)
{
    ...
    if ((int)gif->frame_holders <= frame)
    {
        ...
        if (temp_buf == NULL) { return GIF_INSUFFICIENT_MEMORY; }
        gif->frames = temp_buf;
    ...
    if (gif->decoded_frame == frame) { gif->decoded_frame = GIF_INVALID_FRAME; }
    ...
    if (return_value != GIF_OK) { return return_value; }
    ...
    if (gif_initialise_sprite(gif, (offset_x + width), (offset_y + height))) { return GIF_INSUFFICIENT_MEMORY; }
    ...
    if (gif_data[0] > LZW_CODE_MAX) { return GIF_DATA_ERROR; }
    ...
    while (block_size != 1)
    {
        ...
        if ((int)(gif_bytes - block_size) < 0)
        {
            ...
            else { return GIF_INSUFFICIENT_FRAME_DATA; }
        }
        else
        {
...
static gif_result gif_skip_frame_extensions(gif_animation* gif)
{
    ...
    {
        ++gif_data;
        if (gif_data >= gif_end) { return GIF_INSUFFICIENT_FRAME_DATA; }
        ...
        default:
             ...
             */
            if (gif_data + 1 >= gif_end) { return GIF_INSUFFICIENT_FRAME_DATA; }
            gif_data += (2 + gif_data[1]);
        }
...
static void gif__record_previous_frame(gif_animation* gif)
{
    ...
    if (gif->decoded_frame == GIF_INVALID_FRAME || gif->decoded_frame == gif->prev_index)
    {
    ...
    if (gif->prev_frame != NULL && gif->width * gif->height > gif->prev_width * gif->prev_height) { need_alloc = true; }

    if (need_alloc)
    {
        prev_frame = realloc(gif->prev_frame, gif->width * gif->height * 4);
        if (prev_frame == NULL) { return; }
    }
    else { prev_frame = gif->prev_frame; }
...
static gif_result gif__recover_previous_frame(const gif_animation* gif)
{
    ...
    if (prev_frame == NULL) { return GIF_FRAME_DATA_ERROR; }

    assert(gif->bitmap_callbacks.bitmap_get_buffer);
    frame_data = (void*)gif->bitmap_callbacks.bitmap_get_buffer(gif->frame_image);
    if (!frame_data) { return GIF_INSUFFICIENT_MEMORY; }
...
static gif_result gif_internal_decode_frame(gif_animation* gif, unsigned int frame, bool clear_image)
{
    ...
    if (gif->frames[frame].display == false) { return GIF_OK; }

    /* Ensure the frame is in range to decode */
    if (frame > gif->frame_count_partial) { return GIF_INSUFFICIENT_DATA; }

    /* done if frame is already decoded */
    if ((!clear_image) && ((int)frame == gif->decoded_frame)) { return GIF_OK; }
    ...
    if ((return_value = gif_skip_frame_extensions(gif)) != GIF_OK) { goto gif_decode_frame_exit; }
    gif_data = (gif->gif_data + gif->buffer_position);
    gif_bytes = (gif_end - gif_data);
    ...
    if (flags & GIF_COLOUR_TABLE_MASK)
    {
        ...
        else { gif_data += 3 * colour_table_size; }
        gif_bytes = (gif_end - gif_data);
    }
    else { colour_table = gif->global_colour_table; }
    ...
    if (!clear_image)
    {
        ...
        else if ((frame != 0) && (gif->frames[frame - 1].disposal_method == GIF_FRAME_CLEAR))
        {
            ...
            if (return_value != GIF_OK) { goto gif_decode_frame_exit; }
        }
        ...
        res = lzw_decode_init(gif->lzw_ctx, gif->gif_data, gif->buffer_size, gif->buffer_position, gif_data[0],
                              &stack_base, &stack_pos);
        if (res != LZW_OK) { return gif_error_from_lzw(res); }

        /* Decompress the data */
        for (y = 0; y < height; y++)
        {
            if (interlace) { decode_y = gif_interlaced_line(height, y) + offset_y; }
            else { decode_y = y + offset_y; }
            ...
            while (x > 0)
            {
                ...
                if (burst_bytes > 0)
                {
                    ...
                    {
                        colour = *--stack_pos;
                        if (((gif->frames[frame].transparency) && (colour != gif->frames[frame].transparency_index)) ||
                ...
                else
                {
                    ...
                    if (res != LZW_OK)
                    {
                        ...
                        if (res == LZW_OK_EOD) { return_value = GIF_OK; }
                        else { return_value = gif_error_from_lzw(res); }
                        goto gif_decode_frame_exit;
                    }
                }
            }
    ...
    else
    {
        ...
        if (gif->frames[frame].disposal_method == GIF_FRAME_CLEAR)
        {
            for (y = 0; y < height; y++)
            {
                ...
                if (gif->frames[frame].transparency) { memset(frame_scanline, GIF_TRANSPARENT_COLOUR, width * 4); }
                else { memset(frame_scanline, colour_table[gif->background_index], width * 4); }
            }
        }
    }
gif_decode_frame_exit:
    ...
    if (gif->frames[frame].virgin)
    {
        ...
        else { gif->frames[frame].opaque = false; }
    ...
    if (gif->bitmap_callbacks.bitmap_modified) { gif->bitmap_callbacks.bitmap_modified(gif->frame_image); }
...
gif_result gif_initialise(gif_animation* gif, size_t size, unsigned char* data)
{
     ...
     */
    if (gif->buffer_size < GIF_STANDARD_HEADER_SIZE) { return GIF_INSUFFICIENT_DATA; }
    ...
    if (gif->buffer_position == 0)
    {
         ...
         */
        if (strncmp((const char*)gif_data, "GIF", 3) != 0) { return GIF_DATA_ERROR; }
        gif_data += 3;
        ...
        */
         ...
         */
        if (((gif->width == 640) && (gif->height == 480)) || ((gif->width == 640) && (gif->height == 512)) ||
            ((gif->width == 800) && (gif->height == 600)) || ((gif->width == 1024) && (gif->height == 768)) ||
            ((gif->width == 1280) && (gif->height == 1024)) || ((gif->width == 1600) && (gif->height == 1200)) ||
            ((gif->width == 0) || (gif->height == 0)) || ((gif->width > 2048) || (gif->height > 2048)))
        {
            gif->width = 1;
            gif->height = 1;
        }
        ...
        {
            if (gif_data[0] == GIF_TRAILER) { return GIF_OK; }
            else { return GIF_INSUFFICIENT_DATA; }
        }
    ...
    if ((return_value == GIF_INSUFFICIENT_MEMORY) || (return_value == GIF_DATA_ERROR)) { return return_value; }
