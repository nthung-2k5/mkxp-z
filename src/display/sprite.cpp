...
struct SpritePrivate
{
    ...
    SpritePrivate():
        bitmap(0), srcRect(&tmp.rect), mirrored(false), bushDepth(0), efBushDepth(0), bushOpacity(128), opacity(255),
        blendType(BlendNormal), pattern(0), patternTile(true), patternOpacity(255), invert(false), isVisible(false),
        color(&tmp.color), tone(&tmp.tone)

    {
        ...
        prepareCon = shState->prepareDraw.connect(&SpritePrivate::prepare, this);
    ...
    void onSrcRectChange()
    {
        ...
        if (!nullOrDisposed(bitmap)) bmSize = Vec2i(bitmap->width(), bitmap->height());
    ...
    void updateSrcRectCon()
    {
        ...
        srcRectCon.disconnect();
        /* Create new one */
        srcRectCon = srcRect->valueChanged.connect(&SpritePrivate::onSrcRectChange, this);
    }
    ...
    void updateWave()
    {
        ...
        if (lastLength > 0) emitWaveChunk(vert, phase, width, zoomY, firstLength + chunks * 8, lastLength);
...
void Sprite::setZoomY(float value)
{
    ...
    if (rgssVer >= 2) p->wave.dirty = true;
}
...
void Sprite::setPattern(Bitmap* value)
{
    ...
    if (p->pattern == value) return;

    p->pattern = value;

    if (!nullOrDisposed(value)) value->ensureNonMega();
}
...
void Sprite::draw()
{
    ...
    bool renderEffect = p->color->hasEffect() || p->tone->hasEffect() || flashing || p->bushDepth != 0 || p->invert ||
    ...
    if (renderEffect)
    {
        ...
        {
            if (p->pattern->hasHires()) { Debug() << "BUG: High-res Sprite pattern not implemented"; }
        ...
        else { shader.setShouldRenderPattern(false); }
