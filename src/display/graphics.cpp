...
struct Movie
{
    ...
    Movie(bool skippable_): decoder(0), audio(0), video(0), skippable(skippable_), videoBitmap(0), audioThread(0) {}
    ...
    bool preparePlayback()
    {
        ...
        {
            while ((audio = THEORAPLAY_getAudio(decoder)) == NULL &&
                   THEORAPLAY_availableVideo(decoder) < DEF_MAX_VIDEO_FRAMES)
            {
                SDL_Delay(VIDEO_DELAY);
            }
        }
    ...
    void queueAudioPacket(const THEORAPLAY_AudioPacket* audio)
    {
        ...
        if (audioQueueTail) { audioQueueTail->next = item; }
        else { audioQueueHead = item; }
        audioQueueTail = item;
    ...
    void bufferMovieAudio(THEORAPLAY_Decoder* decoder, const Uint32 now)
    {
        ...
        while ((audio = THEORAPLAY_getAudio(decoder)) != NULL)
        {
            ...
            { // don't let this get too far ahead.
                break;
            }
        }
    }
            ...
            while (procBufs--)
            {
                ...
                while (audioQueueHead && (remainingSamples > 0))
                {
                    ...
                    for (ALuint i = 0; i < samplesToProcess; i++)
                    {
                        ...
                        if (val < -1.0f) { *(sampleBuffer++) = SHRT_MIN; }
                        else if (val > 1.0f) { *(sampleBuffer++) = SHRT_MAX; }
                        else { *(sampleBuffer++) = (ALshort)(val * SHRT_MAX); }
                    }
    ...
    void play(float volume)
    {
        ...
        while (THEORAPLAY_isDecoding(decoder))
        {
            ...
            if (openedAudio) { bufferMovieAudio(decoder, now); }
        }
    }
    ...
    ~Movie()
    {
        if (hasAudio)
        {
            if (audioQueueTail) { THEORAPLAY_freeAudio(audioQueueTail->audio); }
            audioQueueTail = NULL;

            if (audioQueueHead) { THEORAPLAY_freeAudio(audioQueueHead->audio); }
            audioQueueHead = NULL;
...
struct MovieOpenHandler: FileSystem::OpenHandler
{
    ...
    MovieOpenHandler(SDL_RWops& srcOps): srcOps(&srcOps) {}
...
struct FPSLimiter
{
    ...
    FPSLimiter(uint16_t desiredFPS):
        lastTickCount(SDL_GetPerformanceCounter()), tickFreq(SDL_GetPerformanceFrequency()),
        tickFreqMS(tickFreq / 1000), tickFreqNS((double)tickFreq / NS_PER_S), disabled(false)
    {
...
struct GraphicsPrivate
{
    ...
    GraphicsPrivate(RGSSThreadData* rtData):
        ...
        winSize(rtData->config.defScreenW, rtData->config.defScreenH), screen(scRes.x, scRes.y), threadData(rtData),
        glCtx(SDL_GL_GetCurrentContext()), multithreadedMode(true), frameRate(DEF_FRAMERATE), frameCount(0),
        brightness(255), fpsLimiter(frameRate), useFrameSkip(rtData->config.frameSkip), frozen(false), last_update(0),
        last_avg_update(0), backingScaleFactor(1), integerScaleFactor(0, 0),
    ...
    {
        Vec2i newScale(findHighestFittingScale(scRes.x, winSize.x), findHighestFittingScale(scRes.y, winSize.y));
        ...
        if (newScale == integerScaleFactor) return false;
    ...
    void rebuildIntegerScaleBuffer()
    {
        ...
        TEXFBO::allocEmpty(integerScaleBuffer, scRes.x * integerScaleFactor.x, scRes.y * integerScaleFactor.y);
        TEXFBO::linkFBO(integerScaleBuffer);
    }

    bool integerScaleStepApplicable() const
    {
        if (!integerScaleActive) return false;
    ...
    void checkResize(bool skipIntScaleBuffer = false)
    {
        if (threadData->windowSizeMsg.poll(winSize))
        {
            ...
            if (integerScaleActive && findHighestIntegerScale() && !skipIntScaleBuffer) rebuildIntegerScaleBuffer();
    ...
    void compositeToBuffer(TEXFBO& buffer) { compositeToBufferScaled(buffer, scRes.x, scRes.y); }
    ...
    void redrawScreen()
    {
        ...
        if (integerScaleStepApplicable())
        {
            ...
            GLMeta::blitRectangle(IntRect(0, 0, scRes.x, scRes.y),
                                  IntRect(0, 0, integerScaleBuffer.width, integerScaleBuffer.height), false);
        ...
        if (avgFPSData.size() > 40) avgFPSData.erase(avgFPSData.begin());
...
Graphics::Graphics(RGSSThreadData* data)
{
    ...
    else if (data->config.fixedFramerate > 0) { p->fpsLimiter.setDesiredFPS(data->config.fixedFramerate); }
    else if (data->config.fixedFramerate < 0) { p->fpsLimiter.disabled = true; }
}

Graphics::~Graphics() { delete p; }

double Graphics::getDelta() { return shState->runTime() - p->last_update; }

double Graphics::lastUpdate() { return p->last_update; }
...
void Graphics::update(bool checkForShutdown)
{
    ...
    if (checkForShutdown) p->checkShutDownReset();

    p->checkSyncLock();

#ifdef MKXPZ_STEAM
    if (STEAMSHIM_alive()) STEAMSHIM_pump();
#endif
...
void Graphics::transition(int duration, const char* filename, int vague)
{
    ...
    if (transMap)
    {
        ...
        if (transMap->hasHires()) { Debug() << "BUG: High-res Graphics transMap not implemented"; }
...
double Graphics::averageFrameRate() { return p->averageFPS(); }
...
{
    return p->screen.getPP().frontBuffer().fbo == FBO::boundFramebufferID ||
           p->screen.getPP().backBuffer().fbo == FBO::boundFramebufferID;
}

int Graphics::displayContentWidth() const { return p->scSize.x; }

int Graphics::displayContentHeight() const { return p->scSize.y; }
...
void Graphics::resizeScreen(int width, int height)
{
    ...
    if (p->integerScaleActive) p->rebuildIntegerScaleBuffer();
...
void Graphics::resizeWindow(int width, int height, bool center)
{
    ...
    if (width == p->winSize.x / p->backingScaleFactor && height == p->winSize.y / p->backingScaleFactor) return;
...
bool Graphics::updateMovieInput(Movie* movie) { return p->threadData->rqTerm || p->threadData->rqReset; }

void Graphics::playMovie(const char* filename, int volume_, bool skippable)
{
    if (shState->config().enableHires) { Debug() << "BUG: High-res Graphics playMovie not implemented"; }
...
bool Graphics::getFullscreen() const { return p->threadData->ethread->getFullscreen(); }

void Graphics::setFullscreen(bool value) { p->threadData->ethread->requestFullscreenMode(value); }

bool Graphics::getShowCursor() const { return p->threadData->ethread->getShowCursor(); }

void Graphics::setShowCursor(bool value) { p->threadData->ethread->requestShowCursor(value); }
...
void Graphics::setSmoothScaling(int value) { shState->config().smoothScaling = value; }

bool Graphics::getIntegerScaling() const { return p->integerScaleActive; }
...
bool Graphics::getLastMileScaling() const { return p->integerLastMileScaling; }
...
bool Graphics::getThreadsafe() const { return p->multithreadedMode; }

void Graphics::setThreadsafe(bool value) { p->multithreadedMode = value; }
...
void Graphics::repaintWait(const AtomicFlag& exitCond, bool checkReset)
{
    ...
    while (!exitCond)
    {
        ...
        if (checkReset) shState->checkReset();
...
void Graphics::lock(bool force) { p->setLock(force); }

void Graphics::unlock(bool force) { p->releaseLock(force); }
