...
#define BTN_STRING_CUSTOM(btn, name)                                                                                   \
    {                                                                                                                  \
        Input::btn, name                                                                                               \
    }
...
struct VButton
{
    ...
    const char* str;
} static vButtons[] = {BTN_STRING(Up),    BTN_STRING(Down), BTN_STRING(L), BTN_STRING(Left),
                       BTN_STRING(Right), BTN_STRING(R),    BTN_STRING(A), BTN_STRING(B),
                       BTN_STRING(C),     BTN_STRING(X),    BTN_STRING(Y), BTN_STRING(Z)};
...
std::string sourceDescString(const SourceDesc& src)
{
    ...
    case CAxis:
        snprintf(buf, sizeof(buf), "%s%c", shState->input().getAxisName(src.d.ca.axis),
...
struct BindingWidget: Widget
{
    ...
    BindingWidget(int vbIndex, SMP* p, const IntRect& rect): Widget(p, rect), vb(vButtons[vbIndex]), hoveredCell(-1) {}
...
struct Button: Widget
{
    ...
    Button(SMP* p, const IntRect& rect, const char* str, Callback cb): Widget(p, rect), str(str), cb(cb), hovered(false)
    {
    }
...
struct Label: Widget
{
    ...
    Label(SMP* p, const IntRect& rect, const char* str, uint8_t r, uint8_t g, uint8_t b):
...
struct SettingsMenuPrivate
{
    ...
    void blitTextSurf(SDL_Surface* surf, int x, int y, int alignW, SDL_Surface* txtSurf, Justification just)
    {
        ...
        if (txtSurf->w <= alignW) { SDL_BlitSurface(txtSurf, 0, surf, &dstRect); }
        else
        {
            dstRect.w = alignW;
            dstRect.x = x;
    ...
    void drawText(SDL_Surface* surf, const char* str, int x, int y, int alignW, Justification just, SDL_Color c,
                  bool bold = false)
    {
        ...
        for (size_t i = 0; i < bWidgets.size(); ++i)
        {
            for (size_t j = 0; j < 4; ++j)
            {
                ...
                for (size_t k = 0; k < bWidgets.size(); ++k)
                {
                    ...
                    {
                        if (bWidgets[k].src[l] != src) continue;
    ...
    void onMotion(const SDL_MouseMotionEvent& e)
    {
        ...
        if (w != hovered)
        {
            if (hovered) hovered->leave();
            hovered = w;
        }

        if (hovered) hovered->motion(e.x, e.y);
    }
    ...
    bool onCaptureInputEvent(const SDL_Event& event)
    {
        ...
        case SDL_KEYDOWN:
            ...
            if (desc.d.scan == SDL_SCANCODE_RSHIFT) desc.d.scan = SDL_SCANCODE_LSHIFT;

            if (desc.d.scan == SDL_SCANCODE_KP_ENTER) desc.d.scan = SDL_SCANCODE_RETURN;
        ...
        case SDL_CONTROLLERAXISMOTION: {
            ...
            if (v > -JAXIS_THRESHOLD && v < JAXIS_THRESHOLD) return true;
...
void Widget::click(int x, int y, uint8_t button) { clickHandler(x - rect.x, y - rect.y, button); }
...
void BindingWidget::drawHandler(SDL_Surface* surf)
{
    ...
    const int cellOff[] = {cellOffX, 1, cellOffX + cellW, 1, cellOffX, cellH, cellOffX + cellW, cellH};

    const int lbOff[] = {0, cellH / 2, cellW, cellH / 2, 0, cellH + cellH / 2, cellW, cellH + cellH / 2};

    /* Hovered cell background */
    if (hoveredCell != -1)
        p->fillRect(surf, cBgDark, cellOff[hoveredCell * 2], cellOff[hoveredCell * 2 + 1], cellW, cellH);
    ...
    for (size_t i = 0; i < 4; ++i)
    {
        ...
        p->strokeRectInner(surf, cellOff[i * 2] + 1, cellOff[i * 2 + 1] + 1, cellW - 2, cellH - 3, 1, 255, 0, 0);
    }
}
...
void BindingWidget::motionHandler(int x, int y) { setHoveredCell(cellIndex(x, y)); }
...
void BindingWidget::appendBindings(BDescVec& d) const
{
    for (size_t i = 0; i < 4; ++i)
    {
        if (src[i].type == Invalid) continue;
...
{
    if (hovered) p->fillRect(surf, cBgDark, 0, 0, rect.w, rect.h);
...
{
    if (visible) p->drawText(surf, str, 0, rect.h / 2, rect.w, Left, c);
}
...
#undef SET_BUTTON_NAME
    ...
    p->window = SDL_CreateWindow("Key bindings", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, winSize.x, winSize.y,
    ...
    IntRect btRects[] = {IntRect(16, buttonY, 112, buttonH), IntRect(winSize.x - 16 - 64 * 2 - 8, buttonY, 64, buttonH),
...
bool SettingsMenu::onEvent(const SDL_Event& event)
{
    ...
    case SDL_KEYUP:
        ...
        if (event.window.windowID != p->winID) return false;
        break;

    case SDL_JOYBUTTONDOWN:
    case SDL_CONTROLLERBUTTONUP:
    case SDL_CONTROLLERAXISMOTION:
        if (!p->hasFocus) return false;
        break;
    ...
    case SDL_CONTROLLERBUTTONDOWN:
    case SDL_CONTROLLERAXISMOTION:
        if (p->state != AwaitingInput) return true;
        break;
    ...
    if (p->state == AwaitingInput) return p->onCaptureInputEvent(event);
