...
std::string systemImpl::getUserName() { return std::string(NSUserName().UTF8String); }

int systemImpl::getScalingFactor() { return NSApplication.sharedApplication.mainWindow.backingScaleFactor; }

bool systemImpl::isWine() { return false; }
...
systemImpl::WineHostType systemImpl::getRealHostType() { return WineHostType::Mac; }
...
{
    if (@available(macOS 10.13.0, *)) { return MTLCreateSystemDefaultDevice() != nil; }
    return false;
}
...
std::string getPlistValue(const char* key)
{
    ...
    if (hash != nil) { return std::string(hash.UTF8String); }
    return "";
}
