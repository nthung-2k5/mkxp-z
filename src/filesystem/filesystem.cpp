...
static jobject getJNIGlobalContext(JNIEnv* env)
{
    ...
    jmethodID midCurrentActivityThread =
        env->GetStaticMethodID(clsActivityThread, "currentActivityThread", "()Landroid/app/ActivityThread;");
...
struct SDLRWIoContext
{
    ...
    SDLRWIoContext(const char* filename): ops(SDL_RWFromFile(filename, "r")), filename(filename)
    {
        if (!ops) throw Exception(Exception::SDLError, "Failed to open file: %s", SDL_GetError());
    }
...
static SDL_RWops* getSDLRWops(PHYSFS_Io* io) { return static_cast<SDLRWIoContext*>(io->opaque)->ops; }
...
static PHYSFS_sint64 SDLRWIoTell(struct PHYSFS_Io* io) { return SDL_RWseek(getSDLRWops(io), 0, RW_SEEK_CUR); }
...
static inline PHYSFS_File* sdlPHYS(SDL_RWops* ops) { return static_cast<PHYSFS_File*>(ops->hidden.unknown.data1); }
...
static void throwPhysfsError(const char* desc)
{
    ...
    else { englishStr = PHYSFS_getErrorByCode(ec); }
...
FileSystem::FileSystem(const char* argv0, bool allowSymlinks)
{
    ...
    if (PHYSFS_init((char*)&ainit) == 0) throwPhysfsError("Error initializing PhysFS");
#else
    if (PHYSFS_init(argv0) == 0) throwPhysfsError("Error initializing PhysFS");
#endif
    ...
    if (er == 0) throwPhysfsError("Error registering PhysFS RGSS archiver");
...
void FileSystem::addPath(const char* path, const char* mountpoint, bool reload)
{
    ...
    if (!state)
    {
        ...
        if (io) state = PHYSFS_mountIo(io, path, 0, 1);
    }
    if (!state)
    {
...
static PHYSFS_EnumerateCallbackResult findFontsFolderCB(void* data, const char*, const char* fname)
{
    size_t i = 0;
    char buffer[512];
    const char* s = fname;
    ...
    if (strcmp(buffer, "fonts") == 0) PHYSFS_enumerate(fname, fontSetEnumCB, data);
...
struct OpenReadEnumData
{
    ...
    OpenReadEnumData(FileSystem::OpenHandler& handler, const char* filename, size_t filenameN,
                     BoostHash<std::string, std::string>* pathTrans):
        handler(handler),
        filename(filename), filenameN(filenameN), pathTrans(pathTrans), matchCount(0), stopSearching(false),
        physfsError(0)
    {
    }
};

static PHYSFS_EnumerateCallbackResult openReadEnumCB(void* d, const char* dirpath, const char* filename)
{
    ...
    if (strncmp(filename, data.filename, data.filenameN) != 0) return PHYSFS_ENUM_OK;

    if (!*dirpath) { fullPath = filename; }
    else
    {
     ...
     * found our file */
    if (last != '.' && last != '\0') return PHYSFS_ENUM_OK;
...
void FileSystem::openRead(OpenHandler& handler, const char* filename)
{
    ...
    else { PHYSFS_enumerate(dir, openReadEnumCB, &data); }

    if (data.physfsError) throw Exception(Exception::PHYSFSError, "PhysFS: %s", data.physfsError);

    if (data.matchCount == 0) throw Exception(Exception::NoFileError, "%s", filename);
}

void FileSystem::openReadRaw(SDL_RWops& ops, const char* filename, bool freeOnClose)
{
...
bool FileSystem::exists(const char* filename) { return PHYSFS_exists(normalize(filename, false, false).c_str()); }
...
const char* FileSystem::desensitize(const char* filename)
{
    ...
    if (p->havePathCache && p->pathCache.contains(fn_lower)) return p->pathCache[fn_lower].c_str();
    return filename;
}
