...
struct Vec4
{
    ...
    bool xyzNotNull() const { return (x != 0.0f || y != 0.0f || z != 0.0f); }
};
...
struct Vec2i
{
    ...
    Vec2i operator+(const Vec2i& value) const { return Vec2i(x + value.x, y + value.y); }
    ...
    Vec2i operator%(int value) const { return Vec2i(x % value, y % value); }

    Vec2i operator&(unsigned value) const { return Vec2i(x & value, y & value); }
...
struct IntRect: SDL_Rect
{
    ...
    Vec2i pos() const { return Vec2i(x, y); }
    ...
    bool encloses(const IntRect& o) const { return (x <= o.x && y <= o.y && x + w >= o.x + o.w && y + h >= o.y + o.h); }
};
...
struct FloatRect
{
    ...
    FloatRect(const StaticRect& d): x(d.x), y(d.y), w(d.w), h(d.h) {}

    FloatRect(const IntRect& r): x(r.x), y(r.y), w(r.w), h(r.h) {}
...
struct NormValue
{
    ...
    bool operator==(int value) const { return unNorm == clamp(value, 0, 255); }

    operator int() const { return unNorm; }
};
