...
constexpr static uint32_t LCG_TABLE[30][2] = {
    {0x00000007, 0x00000003}, {0x00000031, 0x00000018}, {0x00000961, 0x000004b0}, {0x0057f6c1, 0x002bfb60},
    {0xa5057d81, 0xd282bec0}, {0x6e913b01, 0x37489d80}, {0xc0bb7601, 0x605dbb00}, {0x1bdaec01, 0x8ded7600},
    {0x0145d801, 0x80a2ec00}, {0x28cbb001, 0x1465d800}, {0xea976001, 0x754bb000}, {0x392ec001, 0x1c976000},
    {0x025d8001, 0x012ec000}, {0x44bb0001, 0x225d8000}, {0x89760001, 0xc4bb0000}, {0x12ec0001, 0x89760000},
    {0x25d80001, 0x12ec0000}, {0x4bb00001, 0x25d80000}, {0x97600001, 0x4bb00000}, {0x2ec00001, 0x97600000},
    {0x5d800001, 0x2ec00000}, {0xbb000001, 0x5d800000}, {0x76000001, 0xbb000000}, {0xec000001, 0x76000000},
    {0xd8000001, 0xec000000}, {0xb0000001, 0xd8000000}, {0x60000001, 0xb0000000}, {0xc0000001, 0x60000000},
...
struct RGSS_entryHandle
{
    ...
    RGSS_entryHandle(const RGSS_entryData& data, PHYSFS_Io* archIo):
        data(data), currentMagic(data.startMagic), currentOffset(0)
    {
...
static bool readUint32(PHYSFS_Io* io, uint32_t& result)
{
    ...
    result = ((buff[0] << 0x00) & 0x000000FF) | ((buff[1] << 0x08) & 0x0000FF00) | ((buff[2] << 0x10) & 0x00FF0000) |
...
#define PHYSFS_ALLOC(type) static_cast<type*>(PHYSFS_getAllocator()->Malloc(sizeof(type)))
...
static inline uint32_t advanceMagicN(uint32_t& magic, uint32_t n)
{
    uint32_t old = magic;
    int table_index = 0;
    ...
    while (n != 0)
    {
        if (n & 1) { magic = magic * LCG_TABLE[table_index][0] + LCG_TABLE[table_index][1]; }
        n >>= 1;
        table_index++;
    }
...
static PHYSFS_sint64 RGSS_ioRead(PHYSFS_Io* self, void* buffer, PHYSFS_uint64 len)
{
    ...
    if (preAlign > 0)
    {
        ...
        if ((offs + preAlign) % 4 == 0) advanceMagic(entry->currentMagic);
    }
...
static int RGSS_ioSeek(PHYSFS_Io* self, PHYSFS_uint64 offset)
{
    ...
    if (offset == entry->currentOffset) return 1;

    if (offset > entry->data.size - 1) return 0;
...
static const PHYSFS_Io RGSS_IoTemplate = {0, /* version */
                                          0, /* opaque */
                                          RGSS_ioRead,
                                          0, /* write */
                                          RGSS_ioSeek,
...
static void processDirectories(RGSS_archiveData* data, BoostSet<std::string>& topLevel, char* nameBuf, uint32_t nameLen)
{
    ...
    for (uint32_t i = 0; i < nameLen; ++i)
    {
        ...
        if (!slash && i + 1 < nameLen) continue;

        if (slash) nameBuf[i] = '\0';
...
static bool verifyHeader(PHYSFS_Io* io, char version)
{
    ...
    if (!IO_READ(io, header, sizeof(header))) return false;

    if (strcmp(header, RGSS_HEADER)) return false;

    if (header[7] != version) return false;
...
static void* RGSS_openArchive(PHYSFS_Io* io, const char*, int forWrite, int* claimed)
{
    ...
    while (true)
    {
        ...
        if (!readUint32(io, nameLen)) break;
...
static PHYSFS_EnumerateCallbackResult RGSS_enumerateFiles(void* opaque, const char* dirname,
                                                          ...
                                                          void* callbackdata)
{
    ...
    if (!data->dirHash.contains(_dirname)) return PHYSFS_ENUM_STOP;
...
static PHYSFS_Io* RGSS_openRead(void* opaque, const char* filename)
{
    ...
    if (!data->entryHash.contains(filename)) return 0;

    RGSS_entryHandle* entry = new RGSS_entryHandle(data->entryHash[filename], data->archiveIo);
...
static int RGSS_stat(void* opaque, const char* filename, PHYSFS_Stat* stat)
{
    ...
    stat->modtime = stat->createtime = stat->accesstime = 0;
    stat->readonly = 1;
...
static PHYSFS_Io* RGSS_noop1(void*, const char*) { return 0; }

static int RGSS_noop2(void*, const char*) { return 0; }

const PHYSFS_Archiver RGSS1_Archiver = {0,
                                        {
                                            "RGSSAD", "RGSS encrypted archive format", "", /* Author */
                                            "",                                            /* Website */
...
const PHYSFS_Archiver RGSS2_Archiver = {0,
                                        {
                                            "RGSS2A", "RGSS2 encrypted archive format", "", /* Author */
                                            "",                                             /* Website */
                                            0                                               /* symlinks not supported */
                                        },
                                        RGSS_openArchive,
...
{
    if (!readUint32(io, result)) return false;
...
static void* RGSS3_openArchive(PHYSFS_Io* io, const char*, int forWrite, int* claimed)
{
    ...
    if (!readUint32(io, baseMagic)) return NULL;
    ...
    while (true)
    {
        ...
        if (!readUint32AndXor(io, offset, baseMagic)) goto error;

        /* Zero offset means entry list has ended */
        if (offset == 0) break;

        if (!readUint32AndXor(io, size, baseMagic)) goto error;

        if (!readUint32AndXor(io, magic, baseMagic)) goto error;

        if (!readUint32AndXor(io, nameLen, baseMagic)) goto error;

        char nameBuf[512];

        if (!IO_READ(io, nameBuf, nameLen)) goto error;
...
const PHYSFS_Archiver RGSS3_Archiver = {0,
                                        {
                                            "RGSS3A", "RGSS3 encrypted archive format", "", /* Author */
                                            "",                                             /* Website */
                                            0                                               /* symlinks not supported */
                                        },
                                        RGSS3_openArchive,
