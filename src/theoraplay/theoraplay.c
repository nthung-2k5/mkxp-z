...
typedef unsigned char* (*ConvertVideoFrameFn)(const th_info* tinfo, const th_ycbcr_buffer ycbcr);

static unsigned char* ConvertVideoFrame420ToYUVPlanar(const th_info* tinfo, const th_ycbcr_buffer ycbcr, const int p0,
...
static unsigned char* ConvertVideoFrame420ToYV12(const th_info* tinfo, const th_ycbcr_buffer ycbcr)
{
    return ConvertVideoFrame420ToYUVPlanar(tinfo, ycbcr, 0, 2, 1);
} // ConvertVideoFrame420ToYV12

static unsigned char* ConvertVideoFrame420ToIYUV(const th_info* tinfo, const th_ycbcr_buffer ycbcr)
{
...
{
    ctx->worker = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)routine, (LPVOID)ctx, 0, NULL);
...
static inline void Mutex_Destroy(THEORAPLAY_MUTEX_T mutex) { CloseHandle(mutex); }

static inline void Mutex_Lock(THEORAPLAY_MUTEX_T mutex) { WaitForSingleObject(mutex, INFINITE); }

static inline void Mutex_Unlock(THEORAPLAY_MUTEX_T mutex) { ReleaseMutex(mutex); }
#else
static inline int Thread_Create(TheoraDecoder* ctx, void* (*routine)(void*))
{
...
static inline void Thread_Join(THEORAPLAY_THREAD_T thread) { pthread_join(thread, NULL); }

static inline int Mutex_Create(TheoraDecoder* ctx) { return pthread_mutex_init(&ctx->lock, NULL); }

static inline void Mutex_Destroy(THEORAPLAY_MUTEX_T mutex) { pthread_mutex_destroy(&mutex); }

static inline void Mutex_Lock(THEORAPLAY_MUTEX_T mutex) { pthread_mutex_lock(&mutex); }

static inline void Mutex_Unlock(THEORAPLAY_MUTEX_T mutex) { pthread_mutex_unlock(&mutex); }
#endif
...
#define queue_ogg_page(ctx)                                                                                            \
    ...
    while (!ctx->halt && bos)
    {
        if (FeedMoreOggData(ctx->io, &sync) <= 0) goto cleanup;
    ...
    // no audio OR video?
    if (ctx->halt || (!vpackets && !tpackets)) goto cleanup;
    ...
    while ((!ctx->halt) && ((tpackets && (tpackets < 3)) || (vpackets && (vpackets < 3))))
    {
        ...
        {
            if (ogg_stream_packetout(&tstream, &packet) != 1) break; // get more data?
            if (!th_decode_headerin(&tinfo, &tcomment, &tsetup, &packet)) goto cleanup;
            tpackets++;
        } // while

        while (!ctx->halt && vpackets && (vpackets < 3))
        {
            if (ogg_stream_packetout(&vstream, &packet) != 1) break; // get more data?
            if (vorbis_synthesis_headerin(&vinfo, &vcomment, &packet)) goto cleanup;
            vpackets++;
        } // while
    ...
    if (!ctx->halt && tpackets)
    {
        ...
        if ((tinfo.frame_width > 99999) || (tinfo.frame_height > 99999)) goto cleanup;

        // We treat "unspecified" as NTSC. *shrug*
        if ((tinfo.colorspace != TH_CS_UNSPECIFIED) && (tinfo.colorspace != TH_CS_ITU_REC_470M) &&
        ...
        if (tinfo.fps_denominator != 0) fps = ((double)tinfo.fps_numerator) / ((double)tinfo.fps_denominator);
    ...
    if (!ctx->halt && vpackets)
    {
        ...
        if (!vdsp_init) goto cleanup;
        vblock_init = (vorbis_block_init(&vdsp, &vblock) == 0);
        if (!vblock_init) goto cleanup;
    } // if
    ...
    while (!ctx->halt && !eos)
    {
        ...
        while (!ctx->halt && vpackets)
        {
            ...
            else // no audio available left in current packet?
            {
                ...
                {
                    if (!tpackets) need_pages = 1; // no video, get more pages now.
                    break;                         // we'll get more pages when the video catches up.
                }                                  // if
                else
                {
                    if (vorbis_synthesis(&vblock, &packet) == 0) vorbis_synthesis_blockin(&vdsp, &vblock);
                } // else
            }     // else
        }         // while
...
THEORAPLAY_Decoder* THEORAPLAY_startDecode(THEORAPLAY_Io* io, const unsigned int maxframes,
                                           THEORAPLAY_VideoFormat vidfmt)
{
...
#undef VIDCVT
    ...
    if (ctx == NULL) goto startdecode_failed;
    ...
    if (Mutex_Create(ctx) == 0)
    {
        ...
        if (ctx->thread_created) return (THEORAPLAY_Decoder*)ctx;
    } // if
...
const THEORAPLAY_AudioPacket* THEORAPLAY_getAudio(THEORAPLAY_Decoder* decoder)
{
    ...
    if (retval)
    {
        ...
        retval->next = NULL;
        if (ctx->audiolist == NULL) ctx->audiolisttail = NULL;
    } // if
    Mutex_Unlock(ctx->lock);
...
const THEORAPLAY_VideoFrame* THEORAPLAY_getVideo(THEORAPLAY_Decoder* decoder)
{
    ...
    if (retval)
    {
        ...
        retval->next = NULL;
        if (ctx->videolist == NULL) ctx->videolisttail = NULL;
