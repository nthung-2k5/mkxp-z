...
static unsigned char* THEORAPLAY_CVT_FNNAME_420(const th_info* tinfo, const th_ycbcr_buffer ycbcr)
{
    ...
    if (pixels)
    {
        ...
        for (posy = 0; posy < h; posy++)
        {
            ...
            for (poshalfx = 0; poshalfx < halfw; poshalfx++, posx += 2)
            {
                ...
                const float g1 = (y1 - ((2.0f * (((1.0f - kb) * kb) / ((1.0f - kb) - kr))) * pb) -
                                  ((2.0f * (((1.0f - kr) * kr) / ((1.0f - kb) - kr))) * pr)) *
                                 255.0f;
                const float b1 = (y1 + (2.0f * (1.0f - kb) * pb)) * 255.0f;
                const float r2 = (y2 + (2.0f * (1.0f - kr) * pr)) * 255.0f;
                const float g2 = (y2 - ((2.0f * (((1.0f - kb) * kb) / ((1.0f - kb) - kr))) * pb) -
                                  ((2.0f * (((1.0f - kr) * kr) / ((1.0f - kb) - kr))) * pr)) *
                                 255.0f;
