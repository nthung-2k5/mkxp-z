...
namespace httplib
{
    ...
    struct Response
    {
        ...
        ~Response()
        {
            if (content_provider_resource_releaser_) { content_provider_resource_releaser_(content_provider_success_); }
        }
    ...
    namespace detail
    {
        ...
        inline void duration_to_sec_and_usec(const T& duration, U callback)
        {
            ...
            auto usec =
                std::chrono::duration_cast<std::chrono::microseconds>(duration - std::chrono::seconds(sec)).count();
    ...
    namespace detail
    {
        ...
        inline std::string decode_url(const std::string& s, bool convert_plus_to_space)
        {
            ...
            for (size_t i = 0; i < s.size(); i++)
            {
                ...
                else if (convert_plus_to_space && s[i] == '+') { result += ' '; }
                else { result += s[i]; }
            }
        ...
        {
            if (s.length() >= 2 && s.front() == '"' && s.back() == '"') { return s.substr(1, s.size() - 2); }
            return s;
        }
        ...
        {
            if (glowable_buffer_.empty()) { return fixed_buffer_; }
            else { return glowable_buffer_.data(); }
        }

        inline size_t stream_line_reader::size() const
        {
            if (glowable_buffer_.empty()) { return fixed_buffer_used_size_; }
            else { return glowable_buffer_.size(); }
        }
                               ...
                               BindOrConnect bind_or_connect)
        {
            ...
            for (auto rp = result; rp; rp = rp->ai_next)
            {
                 ...
                 *
                 */
                if (sock == INVALID_SOCKET) { sock = socket(rp->ai_family, rp->ai_socktype, rp->ai_protocol); }
#else
...
#define USE_IF2IP
                                             ...
                                             const std::string& intf, Error& error)
        {
            ...
            if (sock != INVALID_SOCKET) { error = Error::Success; }
            else
            {
        ...
        inline void get_remote_ip_and_port(socket_t sock, std::string& ip, int& port)
        {
            ...
            if (!getpeername(sock, reinterpret_cast<struct sockaddr*>(&addr), &addr_len))
            {
                ...
                if (addr.ss_family == AF_UNIX)
                {
                    ...
                    if (getsockopt(sock, SOL_SOCKET, SO_PEERCRED, &ucred, &len) == 0) { port = ucred.pid; }
#elif defined(SOL_LOCAL) && defined(SO_PEERPID) // __APPLE__
                    pid_t pid;
                    socklen_t len = sizeof(pid);
                    if (getsockopt(sock, SOL_LOCAL, SO_PEERPID, &pid, &len) == 0) { port = pid; }
#endif
                    return;
                }
#endif
        ...
        inline bool gzip_compressor::compress(const char* data, size_t data_length, bool last, Callback callback)
        {
            ...
            do
            {
                ...
                do
                {
                    ...
                    if (!callback(buff.data(), buff.size() - strm_.avail_out)) { return false; }
        ...
        inline bool brotli_compressor::compress(const char* data, size_t data_length, bool last, Callback callback)
        {
            ...
            for (;;)
            {
                ...
                if (output_bytes) { callback(reinterpret_cast<const char*>(buff.data()), output_bytes); }
            }
        ...
        {
            if (decoder_r == BROTLI_DECODER_RESULT_SUCCESS || decoder_r == BROTLI_DECODER_RESULT_ERROR) { return 0; }
        ...
        inline bool parse_header(const char* beg, const char* end, T fn)
        {
            ...
            if (p < end)
            {
                ...
                auto val =
                    compare_case_ignore(key, "Location") ? std::string(p, end) : decode_url(std::string(p, end), false);
        ...
        inline bool read_content_chunked(Stream& strm, T& x, ContentReceiverWithProgress out)
        {
            ...
            while (true)
            {
                ...
                if (!read_content_with_length(strm, chunk_len, nullptr, out)) { return false; }
                          ...
                          ContentReceiverWithProgress receiver, bool decompress)
        {
            return prepare_content_receiver(
                x, status, std::move(receiver), decompress, [&](const ContentReceiverWithProgress& out) {
                    ...
                    if (is_chunked_transfer_encoding(x.headers)) { ret = read_content_chunked(strm, x, out); }
                    else if (!has_header(x.headers, "Content-Length")) { ret = read_content_without_length(strm, out); }
                    else
                    {
                        ...
                        else if (len > 0) { ret = read_content_with_length(strm, len, std::move(progress), out); }
                    }
                                  ...
                                  T is_shutting_down, Error& error)
        {
            ...
            data_sink.write = [&](const char* d, size_t l) -> bool {
                if (ok)
                {
                    if (strm.is_writable() && write_data(strm, d, l)) { offset += l; }
                    else { ok = false; }
                }
                return ok;
            };
                                                 ...
                                                 const T& is_shutting_down)
        {
            ...
            {
                if (!strm.is_writable()) { return false; }
                else if (!content_provider(offset, 0, data_sink)) { return false; }
                                          ...
                                          const T& is_shutting_down, U& compressor, Error& error)
        {
            ...
            auto done_with_trailer = [&](const Headers* trailer) {
                ...
                if (!write_data(strm, done_marker.data(), done_marker.size())) { ok = false; }
                ...
                if (trailer)
                {
                    for (const auto& kv: *trailer)
                    {
                        ...
                        if (!write_data(strm, field_line.data(), field_line.size())) { ok = false; }
                    }
                }
        ...
        inline void parse_query_text(const std::string& s, Params& params)
        {
            ...
            split(s.data(), s.data() + s.size(), '&', [&](const char* b, const char* e) {
                ...
                split(b, e, '=', [&](const char* b2, const char* e2) {
                    if (key.empty()) { key.assign(b2, e2); }
                    else { val.assign(b2, e2); }
                });

                if (!key.empty()) { params.emplace(decode_url(key, true), decode_url(val, true)); }
            });
        }
        ...
        inline void parse_disposition_params(const std::string& s, Params& params)
        {
            ...
            split(s.data(), s.data() + s.size(), ';', [&](const char* b, const char* e) {
                ...
                if (!key.empty()) { params.emplace(trim_double_quotes_copy((key)), trim_double_quotes_copy((val))); }
            });
        }
        ...
        try
        {
            ...
            if (std::regex_match(s, m, re_first_range))
            {
                ...
                split(&s[pos], &s[pos + len], ',', [&](const char* b, const char* e) {
                    ...
                    {
                        ssize_t first = -1;
                        if (!cm.str(1).empty()) { first = static_cast<ssize_t>(std::stoll(cm.str(1))); }

                        ssize_t last = -1;
                        if (!cm.str(2).empty()) { last = static_cast<ssize_t>(std::stoll(cm.str(2))); }
                        ...
                        while (pos < buf_size())
                        {
                            ...
                            else
                            {
                                ...
                                if (std::regex_match(header, m, re_content_disposition))
                                {
                                    ...
                                    if (it != params.end()) { file_.name = it->second; }
                                    else
                                    {
                                        is_valid_ = false;
                                        return false;
        ...
        inline std::string serialize_multipart_formdata_item_begin(const T& item, const std::string& boundary)
        {
            ...
            if (!item.filename.empty()) { body += "; filename=\"" + item.filename + "\""; }
            body += "\r\n";
        ...
        inline std::pair<size_t, size_t> get_range_offset_and_length(const Request& req, size_t content_length,
                                                                     size_t index)
        {
            auto r = req.ranges[index];

            if (r.first == -1 && r.second == -1) { return std::make_pair(0, content_length); }
        ...
        inline std::pair<size_t, size_t> get_range_offset_and_length(const Request& req, const Response& res,
                                                                     size_t index)
        {
            auto r = req.ranges[index];

            if (r.second == -1) { r.second = static_cast<ssize_t>(res.content_length_) - 1; }
        ...
        inline bool retrieve_root_certs_from_keychain(CFObjectPtr<CFArrayRef>& certs)
        {
            ...
            if (SecTrustCopyAnchorCertificates(&root_security_items) != errSecSuccess) { return false; }
        ...
        inline bool load_system_certs_on_macos(X509_STORE* store)
        {
            ...
            if (retrieve_certs_from_keychain(certs) && certs) { result = add_certs_to_x509_store(certs.get(), store); }
        ...
        inline std::pair<std::string, std::string> make_digest_authentication_header(
            ...
            const std::string& cnonce, const std::string& username, const std::string& password, bool is_proxy = false)
        {
            std::string nc;
            {
                std::stringstream ss;
            ...
            {
                qop = auth.at("qop");
                if (qop.find("auth-int") != std::string::npos) { qop = "auth-int"; }
                else if (qop.find("auth") != std::string::npos) { qop = "auth"; }
                else { qop.clear(); }
            }
            ...
            std::string response;
            {
                ...
                if (qop.empty()) { response = H(H(A1) + ":" + auth.at("nonce") + ":" + H(A2)); }
                else
                {
    ...
    inline void hosted_at(const std::string& hostname, std::vector<std::string>& addrs)
    {
        ...
        for (auto rp = result; rp; rp = rp->ai_next)
        {
            ...
            std::string ip;
            auto dummy = -1;
            if (detail::get_ip_and_port(addr, sizeof(struct sockaddr_storage), ip, dummy)) { addrs.push_back(ip); }
        }
    ...
    {
        if (!detail::has_crlf(key) && !detail::has_crlf(val)) { headers.emplace(key, val); }
    }
    ...
    inline void Response::set_redirect(const std::string& url, int stat)
    {
        if (!detail::has_crlf(url))
        {
            ...
            if (300 <= stat && stat < 400) { this->status = stat; }
    ...
    namespace detail
    {
        ...
        inline PathParamsMatcher::PathParamsMatcher(const std::string& pattern)
        {
            ...
            if (last_param_end < pattern.length()) { static_fragments_.push_back(pattern.substr(last_param_end)); }
        }
        ...
        inline bool PathParamsMatcher::match(Request& request) const
        {
            ...
            for (size_t i = 0; i < static_fragments_.size(); ++i)
            {
                ...
                if (starting_pos + fragment.length() > request.path.length()) { return false; }
    ...
    {
        if (pattern.find("/:") != std::string::npos) { return detail::make_unique<detail::PathParamsMatcher>(pattern); }
        else { return detail::make_unique<detail::RegexMatcher>(pattern); }
    }
            ...
            detail::split(req.target.data(), req.target.data() + req.target.size(), '?',
                          [&](const char* b, const char* e) {
                              ...
                              case 0:
                                  ...
                                  break;
                              case 1: {
                                  if (e - b > 0) { detail::parse_query_text(std::string(b, e), req.params); }
                                  break;
                              }
                              default:
                                  break;
                                            ...
                                            bool need_apply_ranges)
    {
        ...
        if (!res.has_header("Accept-Ranges") && req.method == "HEAD") { res.set_header("Accept-Ranges", "bytes"); }
        ...
        // Response line and headers
        {
            ...
            if (!bstrm.write_format("HTTP/1.1 %d %s\r\n", res.status, status_message(res.status))) { return false; }
        ...
        if (req.method != "HEAD")
        {
            if (!res.body.empty())
            {
                if (!detail::write_data(strm, res.body.data(), res.body.size())) { ret = false; }
            }
                                                    ...
                                                    const std::string& boundary, const std::string& content_type)
    {
        ...
        else
        {
            ...
            else { return detail::write_content_without_length(strm, res.content_provider_, is_shutting_down); }
        }
    }
    ...
    inline bool Server::read_content(Stream& strm, Request& req, Response& res)
    {
        ...
        if (read_content_core(
                ...
                [&](const MultipartFormData& file) {
                    if (file_count++ == CPPHTTPLIB_MULTIPART_FORM_DATA_FILE_MAX_COUNT) { return false; }
                                          ...
                                          MultipartContentHeader multipart_header, ContentReceiver multipart_receiver)
    {
        ...
        if (req.method == "DELETE" && !req.has_header("Content-Length")) { return true; }

        if (!detail::read_content(strm, req, payload_max_length_, res.status, nullptr, out, true)) { return false; }
    ...
    inline int Server::bind_internal(const std::string& host, int port, int socket_flags)
    {
        ...
        if (port == 0)
        {
            struct sockaddr_storage addr;
            socklen_t addr_len = sizeof(addr);
            if (getsockname(svr_sock_, reinterpret_cast<struct sockaddr*>(&addr), &addr_len) == -1) { return -1; }
            if (addr.ss_family == AF_INET) { return ntohs(reinterpret_cast<struct sockaddr_in*>(&addr)->sin_port); }
            else if (addr.ss_family == AF_INET6)
            {
    ...
    {
        if (pre_routing_handler_ && pre_routing_handler_(req, res) == HandlerResponse::Handled) { return true; }
        ...
        // Regular handler
        if (req.method == "GET" || req.method == "HEAD") { return dispatch_request(req, res, get_handlers_); }
        else if (req.method == "POST") { return dispatch_request(req, res, post_handlers_); }
        else if (req.method == "PUT") { return dispatch_request(req, res, put_handlers_); }
        else if (req.method == "DELETE") { return dispatch_request(req, res, delete_handlers_); }
        else if (req.method == "OPTIONS") { return dispatch_request(req, res, options_handlers_); }
        else if (req.method == "PATCH") { return dispatch_request(req, res, patch_handlers_); }
                                     ...
                                     std::string& boundary)
    {
        ...
        if (res.body.empty())
        {
            ...
            {
                size_t length = 0;
                if (req.ranges.empty()) { length = res.content_length_; }
                else if (req.ranges.size() == 1)
                {
                    auto offsets = detail::get_range_offset_and_length(req, res.content_length_, 0);
                ...
                else { length = detail::get_multipart_ranges_data_length(req, res, boundary, content_type); }
            ...
            else
            {
                if (res.content_provider_)
                {
                    if (res.is_chunked_content_provider_)
                    {
                        ...
                        if (type == detail::EncodingType::Gzip) { res.set_header("Content-Encoding", "gzip"); }
                        else if (type == detail::EncodingType::Brotli) { res.set_header("Content-Encoding", "br"); }
                    }
                }
            }
        }
        else
        {
            ...
            else if (req.ranges.size() == 1)
            {
                ...
                if (offset < res.body.size()) { res.body = res.body.substr(offset, length); }
                else
                {
                    res.body.clear();
                    res.status = 416;
            ...
            {
                std::string data;
                if (detail::make_multipart_ranges_data(req, res, boundary, content_type, data)) { res.body.swap(data); }
                else
                {
                    res.body.clear();
                    res.status = 416;
                                        ...
                                        const std::function<void(Request&)>& setup_request)
    {
        ...
        if (req.get_header_value("Connection") == "close") { connection_closed = true; }
        ...
        {
            auto status = 100;
            if (expect_100_continue_handler_) { status = expect_100_continue_handler_(req, res); }
            switch (status)
            {
            case 100:
            case 417:
    ...
    inline ClientImpl::ClientImpl(const std::string& host, int port, const std::string& client_cert_path,
                                  const std::string& client_key_path):
        host_(host),
        port_(port), host_and_port_(adjust_host_string(host) + ":" + std::to_string(port)),
    ...
    inline bool ClientImpl::read_response_line(Stream& strm, const Request& req, Response& res)
    {
        ...
        std::cmatch m;
        if (!std::regex_match(line_reader.ptr(), m, re)) { return req.method == "CONNECT"; }
    ...
    {
        {
            ...
            if (!is_alive)
            {
                ...
                if (is_ssl())
                {
                    ...
                    {
                        auto success = false;
                        if (!scli.connect_with_proxy(socket_, res, success, error)) { return success; }
                    }
        ...
        {
            if (req.headers.find(header.first) == req.headers.end()) { req.headers.insert(header); }
        }
    ...
    inline bool ClientImpl::handle_request(Stream& strm, Request& req, Response& res, bool close_connection,
                                           Error& error)
    {
        if (req.path.empty())
        {
    ...
    inline bool ClientImpl::redirect(Request& req, Response& res, Error& error)
    {
        ...
        if (!port_str.empty()) { next_port = std::stoi(port_str); }
        else if (!next_scheme.empty()) { next_port = next_scheme == "https" ? 443 : 80; }
    ...
    inline bool ClientImpl::write_content_with_provider(Stream& strm, const Request& req, Error& error)
    {
        ...
        if (req.is_chunked_content_provider_)
        {
            ...
            std::unique_ptr<detail::compressor> compressor;
#ifdef CPPHTTPLIB_ZLIB_SUPPORT
            if (compress_) { compressor = detail::make_unique<detail::gzip_compressor>(); }
            else
#endif
            {
    ...
    inline bool ClientImpl::write_request(Stream& strm, Request& req, bool close_connection, Error& error)
    {
        ...
        if (close_connection)
        {
            if (!req.has_header("Connection")) { req.set_header("Connection", "close"); }
        }
        ...
        if (!req.has_header("Host"))
        {
            if (is_ssl())
            {
                if (port_ == 443) { req.set_header("Host", host_); }
                else { req.set_header("Host", host_and_port_); }
            }
            else
            {
                if (port_ == 80) { req.set_header("Host", host_); }
                else { req.set_header("Host", host_and_port_); }
            }
        }
        ...
        {
            if (!req.has_header("Content-Type")) { req.set_header("Content-Type", "text/plain"); }
        ...
        // Body
        if (req.body.empty()) { return write_content_with_provider(strm, req, error); }
    ...
    {
        if (params.empty()) { return Get(path, headers, response_handler, content_receiver, progress); }
    ...
    {
        if (ca_cert_store && ca_cert_store != ca_cert_store_) { ca_cert_store_ = ca_cert_store; }
    }
 ...
 */
    ...
    namespace detail
    {
        ...
        inline SSLSocketStream::SSLSocketStream(socket_t sock, SSL* ssl, time_t read_timeout_sec,
                                                ...
                                                time_t write_timeout_usec):
            sock_(sock),
            ssl_(ssl), read_timeout_sec_(read_timeout_sec), read_timeout_usec_(read_timeout_usec),
            write_timeout_sec_(write_timeout_sec), write_timeout_usec_(write_timeout_usec)
        {
        ...
        {
            if (SSL_pending(ssl_) > 0) { return SSL_read(ssl_, ptr, static_cast<int>(size)); }
            else if (is_readable())
            {
    ...
    inline SSLServer::SSLServer(const char* cert_path, const char* private_key_path,
                                const char* client_ca_cert_file_path, const char* client_ca_cert_dir_path,
    ...
    inline void SSLClient::set_ca_cert_store(X509_STORE* ca_cert_store)
    {
        if (ca_cert_store)
        {
            ...
            else { X509_STORE_free(ca_cert_store); }
        }
    }
    ...
    inline bool SSLClient::load_certs()
    {
        ...
        std::call_once(initialize_cert_, [&]() {
            ...
            {
                if (!SSL_CTX_load_verify_locations(ctx_, ca_cert_file_path_.c_str(), nullptr)) { ret = false; }
            }
            else if (!ca_cert_dir_path_.empty())
            {
                if (!SSL_CTX_load_verify_locations(ctx_, nullptr, ca_cert_dir_path_.c_str())) { ret = false; }
            }
            else
            {
                auto loaded = false;
        ...
        if (alt_names)
        {
            ...
            for (decltype(count) i = 0; i < count && !dsn_matched; i++)
            {
                ...
                if (val->type == type)
                {
                    ...
                    case GEN_IPADD:
                        if (!memcmp(&addr6, name, addr_len) || !memcmp(&addr, name, addr_len)) { ip_matched = true; }
                        break;
                    }
                }
            }
    ...
    inline bool SSLClient::verify_host_with_common_name(X509* server_cert) const
    {
        ...
        if (subject_name != nullptr)
        {
            ...
            if (name_len != -1) { return check_host_name(name, static_cast<size_t>(name_len)); }
        }
                          ...
                          const std::string& client_key_path)
    {
        ...
        if (std::regex_match(scheme_host_port, m, re))
        {
            ...
            else { cli_ = detail::make_unique<ClientImpl>(host, port, client_cert_path, client_key_path); }
        }
    ...
    {
        if (is_ssl_) { static_cast<SSLClient&>(*cli_).set_ca_cert_store(ca_cert_store); }
        else { cli_->set_ca_cert_store(ca_cert_store); }
    }
    ...
    {
        if (is_ssl_) { return static_cast<SSLClient&>(*cli_).get_openssl_verify_result(); }
