...
namespace Encoding
{
    ...
    static std::string getCharset(std::string& str)
    {
        ...
        if (ret.empty()) throw Exception(Exception::MKXPError, "Could not detect string encoding", str.c_str());
        return ret;
    }
    ...
    static std::string convertString(std::string& str)
    {
        ...
        if (!strcmp(charset.c_str(), "UTF-8") || !strcmp(charset.c_str(), "ASCII")) { return std::string(str); }
        ...
        if (result != (size_t)-1 && errno == 0) { buf.resize(buf.size() - outLen); }
        else
        {
