...
namespace json5pp
{
    ...
    class syntax_error: public std::invalid_argument
    {
      public:
        ...
        syntax_error(int ch, const char* context_name):
            std::invalid_argument(std::string("JSON syntax error: ") +
    ...
    namespace impl
    {
        ...
        class manipulator_flags
        {
          public:
            ...
            friend parser<S_ & flags::parse_mask> operator>>(std::istream& istream,
                                                             const manipulator_flags<S_, C_>& manip);

            template <flags_type S_, flags_type C_>
            friend stringifier<S_ & flags::stringify_mask, 0> operator<<(std::ostream& ostream,
                                                                         const manipulator_flags<S_, C_>& manip);
        };
    ...
    class value
    {
      ...
      public:
         ...
         */
        value(const string_type& string): type(TYPE_STRING) { new (&content.string) string_type(string); }

        /**
         * @brief JSON value constructor for "string" type. (const char* version)
         * @param string A string value to be set.
         */
        value(string_p_type string): type(TYPE_STRING) { new (&content.string) string_type(string); }

        /**
         * @brief JSON value constructor for "array" type.
         * @param array An initializer list of elements.
         */
        explicit value(std::initializer_list<value> array): type(TYPE_ARRAY) { new (&content.array) array_type(array); }
         ...
         */
        value(const value& src): type(TYPE_NULL) { *this = src; }
      ...
      public:
        ...
        {
            if (type == TYPE_INTEGER) { return static_cast<number_type>(content.integer); }
            else if (type != TYPE_NUMBER) { throw std::bad_cast(); }
        ...
        {
            if (type == TYPE_NUMBER) { return static_cast<integer_type>(content.number); }
            else if (type != TYPE_INTEGER) { throw std::bad_cast(); }
        ...
        const value& at(const int index, const value& default_value) const
        {
            if (type == TYPE_ARRAY)
            {
                if ((0 <= index) && (index < (int)content.array.size())) { return content.array[index]; }
            }
        ...
        const value& operator[](const int index) const { return at(index); }
        ...
        const value& at(const string_type& key, const value& default_value) const
        {
            if (type == TYPE_OBJECT)
            {
                ...
                if (iter != content.object.end()) { return iter->second; }
            }
            return default_value;
        }
        ...
        const value& operator[](const string_type& key) const { return at(key); }
        ...
        const value& at(const string_p_type key) const { return at(string_type(key)); }

        const value& operator[](const string_p_type key) const { return at(string_type(key)); }
      ...
      public:
        ...
        {
            if (type == TYPE_STRING) { content.string = string; }
            else
            {
                release(TYPE_STRING);
         ...
         * @param string A string to be set.
         */
        value& operator=(string_p_type string) { return (*this = string_type(string)); }
        ...
        {
            if (type == TYPE_ARRAY) { content.array = array; }
            else
            {
                release(TYPE_ARRAY);
        ...
        {
            if (type == TYPE_OBJECT) { content.object = elements; }
            else
            {
                release(TYPE_OBJECT);
                new (&content.object) object_type(elements);
    ...
    namespace impl
    {
        ...
        class parser
        {
          ...
          public:
             ...
             */
            std::istream& operator>>(std::istream& (*manip)(std::istream&)) { return istream >> manip; }
          ...
          private:
             ...
             */
            static constexpr bool has_flag(flags_type flags) { return (F & flags) != 0; }
                        ...
                        if (has_flag(flags::single_line_comment | flags::multi_line_comment))
                        {
                            ...
                            else if (has_flag(flags::multi_line_comment) && (ch == '*'))
                            {
                                ...
                                for (;;)
                                {
                                    ch = istream.get();
                                reeval_asterisk:
                                    if (ch == std::char_traits<char>::eof()) { throw syntax_error(ch, "comment"); }
                                    if (ch != '*') { continue; }
                                    ch = istream.get();
                                    if (ch == '*') { goto reeval_asterisk; }
                                    if (ch == '/') { break; }
                                }
                                continue;
                            }
             ...
             */
            static int to_number(int ch) { return ch - '0'; }
            ...
            {
                if (is_digit(ch)) { return to_number(ch); }
             ...
             * @return false
             */
            static bool is_alpha(int ch) { return (('A' <= ch) && (ch <= 'Z')) || (('a' <= ch) && (ch <= 'z')); }
            ...
            bool equals(int& ch, char expected) { return ((ch = istream.get()) == expected); }
            ...
            void do_parse(value& v)
            {
                ...
                {
                    int ch = skip_spaces();
                    if (ch != std::char_traits<char>::eof()) { throw syntax_error(ch, context); }
                }
            }
            ...
            void parse_value(value& v, const char* context)
            {
                ...
                default:
                    if (is_digit(ch) || (ch == '-') || (ch == '+') || (ch == '.') || (ch == 'i') || (ch == 'N'))
                    {
                        // [number]?
            ...
            void parse_number(value& v, int ch)
            {
                ...
                else if (has_flag(flags::explicit_plus_sign) && (ch == '+')) { ch = istream.get(); }
                // [digit|digits]
                for (;;)
                {
                    if (ch == '0')
                    {
                        ...
                        if (has_flag(flags::hexadecimal) && ((ch == 'x') || (ch == 'X')))
                        {
                            ...
                            if (no_digit) { throw syntax_error(ch, context); }
                    ...
                    else if (has_flag(flags::infinity_number) && (ch == 'i'))
                    {
                        ...
                        {
                            v = negative ? -std::numeric_limits<value::number_type>::infinity()
                ...
                if (frac_part > 0) { number_value += (static_cast<double>(frac_part) * std::pow(10, -frac_divs)); }
                if (exp_part > 0) { number_value *= std::pow(10, exp_negative ? -exp_part : +exp_part); }
            ...
            void parse_string(std::string& buffer, int quote, const char* context)
            {
                ...
                for (;;)
                {
                    int ch = istream.get();
                    if (ch == quote) { break; }
                    else if (ch < ' ') { throw syntax_error(ch, context); }
                    else if (ch == '\\')
                    {
                        // [escape]
                        ch = istream.get();
                        ...
                        {
                        case '\'':
                            if (!has_flag(flags::single_quote)) { throw syntax_error(ch, context); }
                            break;
                        case '"':
                        case '\\':
                        case '/':
                        ...
                        case 'u':
                            // ['u' hex hex hex hex]
                            {
                                ...
                                for (int i = 0; i < 4; ++i)
                                {
                                    ...
                                    if (n < 0) { throw syntax_error(ch, context); }
                                    code = static_cast<char16_t>((code << 4) + n);
                                }
                                if (code < 0x80) { buffer.append(1, (char)code); }
                                else if (code < 0x800)
                                {
                        ...
                        case '\r':
                            ...
                            /* no-break */
                        case '\n':
                            if (has_flag(flags::multi_line_string)) { continue; }
                            /* no-break */
                        default:
                            throw syntax_error(ch, context);
                        }
            ...
            void parse_array(value& v)
            {
                ...
                for (;;)
                {
                    int ch = skip_spaces();
                    if (ch == ']') { break; }
                    if (elements.empty()) { istream.unget(); }
                    else if (ch != ',') { throw syntax_error(ch, context); }
                ...
                if (has_flag(flags::unquoted_key))
                {
                    if ((ch != '"') && (ch != '\''))
                    {
                        for (;; ch = istream.get())
                        {
                            ...
                            else { throw syntax_error(ch, context); }
                            buffer.append(1, (char)ch);
                        }
                        istream.unget();
                        return buffer;
            ...
            void parse_object(value& v)
            {
                ...
                for (;;)
                {
                    ...
                    ch = skip_spaces();
                    if (ch != ':') { throw syntax_error(ch, context); }
                    // [value]
        ...
        class stringifier
        {
          ...
          public:
             ...
             */
            std::ostream& operator<<(std::ostream& (*manip)(std::ostream&)) { return ostream << manip; }
          ...
          private:
             ...
             */
            static const char* get_newline() { return (F & flags::crlf_newline) ? "\r\n" : "\n"; }
            ...
            {
                if (I > 0) { return value::json_type(I, ' '); }
                else if (I < 0) { return value::json_type(-I, '\t'); }
            ...
            void do_stringify(const value& v)
            {
                ...
                {
                  public:
                    fmtsaver(std::ios_base& ios): ios(ios), flags(ios.flags()), width(ios.width()) {}
            ...
            void stringify_value(const value& v, const value::json_type& indent)
            {
                ...
                case value::TYPE_NUMBER:
                    ...
                    {
                        if (!has_flag(flags::not_a_number)) { goto null; }
                        ostream << "NaN";
                    }
                    else if (!std::isfinite(v.content.number))
                    {
                        if (!has_flag(flags::infinity_number)) { goto null; }
                        ostream << ((v.content.number > 0) ? "infinity" : "-infinity");
                    }
                    else { ostream << v.content.number; }
                    break;
                ...
                case value::TYPE_ARRAY:
                    if (v.content.array.empty()) { ostream << "[]"; }
                    else if (I == 0)
                    {
                        const char* delim = "[";
                ...
                case value::TYPE_OBJECT:
                    if (v.content.object.empty()) { ostream << "{}"; }
                    else if (I == 0)
                    {
                        const char* delim = "{";
         ...
         */
        template <flags_type S, flags_type C>
        stringifier<S & flags::stringify_mask, 0> operator<<(std::ostream& ostream,
                                                             const manipulator_flags<S, C>& manip)
        {
        ...
        {
          public:
            explicit imemstream(const void* data, std::size_t size): std::istream(&buf), buf(data, size) {}
     ...
     * @return A new parser
     */
    inline impl::parser<0> operator>>(std::istream& istream, value& v) { return impl::parser<0>(istream) >> v; }
    ...
    inline value parse(std::istream& istream, bool finished = true)
    {
        ...
        value v;
        if (finished) { parser<flags::finished>(istream) >> v; }
        else { parser<0>(istream) >> v; }
        return v;
    }
    ...
    inline value parse5(std::istream& istream, bool finished = true)
    {
        using namespace impl;
        value v;
        if (finished) { parser<flags::json5_rules | flags::finished>(istream) >> v; }
        else { parser<flags::json5_rules>(istream) >> v; }
        return v;
    }
