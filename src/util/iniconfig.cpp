...
bool INIConfiguration::load(std::istream& is)
{
    ...
    while (!is.eof() && !is.bad())
    {
        if (line[0] == '[') { currSectionName = line.substr(1, line.find_last_of(']') - 1); }
...
std::string INIConfiguration::getStringProperty(const std::string& sname, const std::string& name,
                                                const std::string& def) const
{
    ...
    if (sectionIt != m_SectionMap.end())
    {
        ...
        if (sectionIt->second.getStringProperty(name, prop)) { return prop; }
    }
