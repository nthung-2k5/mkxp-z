...
struct IntruListLink
{
    ...
    IntruListLink(T* data): prev(0), next(0), data(data) {}
...
class IntruList
{
  ...
  public:
    ...
    void insertBefore(IntruListLink<T>& node, IntruListLink<T>& prev)
    {
        node.next = &prev;
        node.prev = prev.prev;
        prev.prev->next = &node;
    ...
    IntruListLink<T>* begin() { return root.next; }

    IntruListLink<T>* end() { return &root; }
