...
namespace sigslot
{
    ...
    namespace trait
    {
        ...
        constexpr bool is_observer_v =
            std::is_base_of<::sigslot::detail::observer_type, std::remove_pointer_t<T>>::value;
    ...
    namespace detail
    {
        ...
        template <typename T>
        struct object_pointer<T, std::enable_if_t<!trait::is_pointer_v<T> && !trait::is_weak_ptr_v<T> &&
        ...
        class copy_on_write
        {
          ...
          public:
            ...
            copy_on_write(const copy_on_write& x) noexcept: m_data(x.m_data) { ++m_data->count; }

            copy_on_write(copy_on_write&& x) noexcept: m_data(x.m_data) { x.m_data = nullptr; }

            ~copy_on_write()
            {
                if (m_data && (--m_data->count == 0)) { delete m_data; }
            }

            copy_on_write& operator=(const copy_on_write& x) noexcept
            {
                if (&x != this) { *this = copy_on_write(x); }
                return *this;
            }
 ...
 */
        ...
        {
          public:
            constexpr slot_state(group_id gid) noexcept: m_index(0), m_group(gid), m_connected(true), m_blocked(false)
            {
            }
    ...
    class connection_blocker
    {
      ...
      private:
        ...
        explicit connection_blocker(std::weak_ptr<detail::slot_state> s) noexcept: m_state{std::move(s)}
        {
            if (auto d = m_state.lock()) { d->block(); }
        }

        void release() noexcept
        {
            if (auto d = m_state.lock()) { d->unblock(); }
        }
    ...
    class connection
    {
      ...
      protected:
        ...
        explicit connection(std::weak_ptr<detail::slot_state> s) noexcept: m_state{std::move(s)} {}
    ...
    class scoped_connection final: public connection
    {
      public:
        ...
        scoped_connection(scoped_connection&& o) noexcept: connection{std::move(o.m_state)} {}
      ...
      private:
        ...
        explicit scoped_connection(std::weak_ptr<detail::slot_state> s) noexcept: connection{std::move(s)} {}
    };
    ...
    namespace detail
    {
        ...
        class slot_base: public slot_state
        {
          public:
            ...
            explicit slot_base(cleanable& c, group_id gid): slot_state(gid), cleaner(c) {}
            ...
            void operator()(U&&... u)
            {
                if (slot_state::connected() && !slot_state::blocked()) { call_slot(std::forward<U>(u)...); }
            }
        ...
        class slot final: public slot_base<Args...>
        {
          public:
            ...
            constexpr slot(cleanable& c, F&& f, Gid gid): slot_base<Args...>(c, gid), func{std::forward<F>(f)}
            {
            }
        ...
        class slot_pmf final: public slot_base<Args...>
        {
          public:
            ...
            constexpr slot_pmf(cleanable& c, F&& f, P&& p, group_id gid):
                slot_base<Args...>(c, gid), pmf{std::forward<F>(f)}, ptr{std::forward<P>(p)}
            {
            }
        ...
        class slot_tracked final: public slot_base<Args...>
        {
          public:
            ...
            constexpr slot_tracked(cleanable& c, F&& f, P&& p, group_id gid):
                slot_base<Args...>(c, gid), func{std::forward<F>(f)}, ptr{std::forward<P>(p)}
            {
            }

            bool connected() const noexcept override { return !ptr.expired() && slot_state::connected(); }
          ...
          protected:
            void call_slot(Args... args) override
            {
                ...
                if (slot_state::connected()) { func(args...); }
            }
        ...
        class slot_pmf_tracked final: public slot_base<Args...>
        {
          ...
          protected:
            void call_slot(Args... args) override
            {
                ...
                if (slot_state::connected()) { ((*sp).*pmf)(args...); }
            }
    ...
    class signal_base final: public detail::cleanable
    {
      ...
      public:
        ...
        std::enable_if_t<trait::is_callable_v<arg_list, Pmf, Ptr> && trait::is_observer_v<Ptr>, connection> connect(
            Pmf&& pmf, Ptr&& ptr, group_id gid = 0)
        {
         ...
         */
        template <typename Pmf, typename Ptr>
        std::enable_if_t<trait::is_callable_v<ext_arg_list, Pmf, Ptr> && !trait::is_weak_ptr_compatible_v<Ptr>,
                         connection>
        ...
        std::enable_if_t<trait::is_callable_v<arg_list, Callable> && trait::is_weak_ptr_compatible_v<Trackable>,
                         connection>
        ...
        std::enable_if_t<(trait::is_callable_v<arg_list, Callable> || trait::is_callable_v<ext_arg_list, Callable> ||
                          trait::is_pmf_v<Callable>)&&detail::function_traits<Callable>::is_disconnectable,
                         size_t>
        ...
        {
            return disconnect_if([&](const auto& s) { return s->has_object(obj) && s->has_callable(c); });
        }
      ...
      private:
        ...
        void add_slot(slot_ptr&& s)
        {
            ...
            if (it == groups.end() || it->gid != gid) { it = groups.insert(it, {{}, gid}); }
