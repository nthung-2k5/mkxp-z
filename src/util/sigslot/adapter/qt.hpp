...
namespace sigslot
{
    namespace detail
    {
        ...
        {
            qpointer_adater(T* o) noexcept: m_ptr{o} {}

            void reset() noexcept { m_ptr.clear(); }

            bool expired() const noexcept { return m_ptr.isNull(); }
            ...
            T* lock() const noexcept { return expired() ? nullptr : m_ptr.data(); }
        ...
        {
            qweakpointer_adater(QWeakPointer<T> o) noexcept: m_ptr{std::move(o)} {}

            void reset() noexcept { m_ptr.clear(); }

            bool expired() const noexcept { return m_ptr.isNull(); }

            QSharedPointer<T> lock() const noexcept { return m_ptr.lock(); }
...
template <typename T>
std::enable_if_t<std::is_base_of<QObject, T>::value, sigslot::detail::qpointer_adater<T>> to_weak(T* p)
{
    return {p};
}
