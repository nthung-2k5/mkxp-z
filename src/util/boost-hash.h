...
class BoostHash
{
  ...
  public:
    ...
    inline void remove(const K& key) { p.erase(key); }

    inline const V value(const K& key) const
    {
        const_iterator iter = p.find(key);

        if (iter == p.cend()) return V();
    ...
    inline const V value(const K& key, const V& defaultValue) const
    {
        ...
        if (iter == p.cend()) return defaultValue;

        return iter->second;
    }

    inline V& operator[](const K& key) { return p[key]; }

    inline const_iterator cbegin() const { return p.cbegin(); }

    inline const_iterator cend() const { return p.cend(); }
...
class BoostSet
{
  ...
  public:
    ...
    inline void insert(const K& key) { p.insert(key); }
