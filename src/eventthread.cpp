...
EventThread::EventThread(): ctrl(0), fullscreen(false), showCursor(false) { textInputLock = SDL_CreateMutex(); }

EventThread::~EventThread() { SDL_DestroyMutex(textInputLock); }
...
void EventThread::process(RGSSThreadData& rtData)
{
    ...
    SDL_JoystickUpdate();
    if (SDL_NumJoysticks() > 0 && SDL_IsGameController(0)) { ctrl = SDL_GameControllerOpen(0); }
    ...
    while (true)
    {
        ...
        case SDL_TEXTINPUT:
            lockText(true);
            if (textInputBuffer.size() < 512) { textInputBuffer += event.text.text; }
            lockText(false);
            break;
        ...
        case SDL_CONTROLLERDEVICEADDED:
            if (event.cdevice.which > 0) break;
        ...
        default:
            ...
            case REQUEST_WINCENTER:
                ...
                if (!rc) SDL_SetWindowPosition(win, (dm.w / 2) - (winW / 2), (dm.h / 2) - (winH / 2));
                rtData.rqWindowAdjust.clear();
                break;
            ...
            case REQUEST_MESSAGEBOX: {
                ...
                SDL_ShowSimpleMessageBox(event.user.code, rtData.config.windowTitle.c_str(), message.c_str(), win);
#else
    ...
    if (SDL_GameControllerGetAttached(ctrl)) SDL_GameControllerClose(ctrl);
...
int EventThread::eventFilter(void* data, SDL_Event* event)
{
    ...
    case SDL_APP_WILLENTERBACKGROUND:
        ...
        if (HAVE_ALC_DEVICE_PAUSE) alc.DevicePause(rtData.alcDev);
    ...
    case SDL_APP_DIDENTERFOREGROUND:
        ...
        if (HAVE_ALC_DEVICE_PAUSE) alc.DeviceResume(rtData.alcDev);
...
void EventThread::showMessageBox(const char* body, int flags)
{
    ...
    if (shState->rtData().rqTerm) return;
...
bool EventThread::getShowCursor() const { return showCursor; }

bool EventThread::getControllerConnected() const { return ctrl != 0; }

SDL_GameController* EventThread::controller() const { return ctrl; }
...
void EventThread::lockText(bool lock) { lock ? SDL_LockMutex(textInputLock) : SDL_UnlockMutex(textInputLock); }
...
bool SyncPoint::mainSyncLocked() { return mainSync.locked; }
...
void SyncPoint::Util::lock() { locked.set(); }
